...
**SIMPEL FIGUR MED FX TRE VEKTORER, FX HUND, KAT, KÆLK**
...
man kan oversætte ord til vektorer, der repræsenterer ordenes betydning. Mere præcist kiger vi på en version af algoritmen, der hedder skip-gram.
Word2Vec blev opfundet af en gruppe medarbejdere hos Google i 2013. I
...
@tab-data. Hver række i tabellen består af et inputord og et af dets kontekstord.[^4]
  ...
  : Par af input- og kontekstord {#tab-data}
...
hvordan inputvektoren $\overrightarrow{v}_{w}$ forholder sig til kontekstvektorerne
$\overrightarrow{k}_{c}$ for diverse kontekstord $c$. Vi vil derfor
...
vinklen mellem $\overrightarrow{a}$ og $\overrightarrow{b}$ er $0^\circ$,
...
konteksten udfra, fx \"logre\", der ofte vil forekomme i hunderelaterede kontekster, vil derfor blive repræsenteret med
...
$|\overrightarrow{v}_{\text{logre}}|$ være større end
$|\overrightarrow{v}_{\text{og}}|$ og $|\overrightarrow{v}_{\text{er}}|$.
...
$$
\begin{aligned}
&\overrightarrow{v}_{\text{hund}} =  \begin{pmatrix} 3\\ 2\end{pmatrix}, \quad
\overrightarrow{v}_{\text{kat}} =  \begin{pmatrix} 2\\ 3\end{pmatrix} \\
&\overrightarrow{k}_{\text{madskål}}  =  \begin{pmatrix} 3\\3\end{pmatrix}, \quad
\overrightarrow{k}_{\text{badedragt}} =  \begin{pmatrix} -2\\-1.5 \end{pmatrix}, \quad
\overrightarrow{k}_{\text{lufte}} =  \begin{pmatrix} 2\\0.5 \end{pmatrix}
\end{aligned}
$$
...
$$
\begin{aligned}
&\overrightarrow{v}_{\text{kat}}\cdot \overrightarrow{k}_{\text{madskål}} = \begin{pmatrix} 2\\ 3\end{pmatrix}  \cdot  \begin{pmatrix} 3\\ 3\end{pmatrix} = 2\cdot 3 +  3\cdot 3 = 15 \\ 
&\overrightarrow{v}_{\text{kat}}\cdot \overrightarrow{k}_{\text{badedragt}} = \begin{pmatrix} 2\\ 3\end{pmatrix}  \cdot  \begin{pmatrix} -2\\ -1.5\end{pmatrix} = 2\cdot (-2) +  3\cdot (-1.5) = -8.5 
\end{aligned}
$$
Vi ser, at
$\overrightarrow{v}_{\text{kat}}\cdot \overrightarrow{k}_{\text{madskål}}$ er
større end
$\overrightarrow{v}_{\text{kat}}\cdot \overrightarrow{k}_{\text{badedragt}}$. Det
svarer til, at ordet \"kat\" oftere har \"madskål\" som kontekst, end
det har \"badedragt\". Det ses også ved, at
$\overrightarrow{k}_{\text{madskål}}$ peger i nogenlunde samme retning som
$\overrightarrow{v}_{\text{kat}}$, mens $\overrightarrow{k}_{\text{badedragt}}$ peger
i en helt anden retning.

Vi kan også udregne
$$
\begin{aligned}
&\overrightarrow{v}_{\text{hund}}\cdot \overrightarrow{k}_{\text{madskål}} = \begin{pmatrix} 3\\ 2\end{pmatrix}  \cdot  \begin{pmatrix} 3\\ 3\end{pmatrix} = 3\cdot 3 +  2\cdot 3 = 15
\end{aligned}
$$
Vi ser, at
$\overrightarrow{v}_{\text{hund}}\cdot \overrightarrow{k}_{\text{madskål}} =\overrightarrow{v}_{\text{kat}}\cdot \overrightarrow{k}_{\text{madskål}} =15$,
...
$$
\begin{aligned}
&\overrightarrow{v}_{\text{hund}}\cdot \overrightarrow{k}_{\text{lufte}} = \begin{pmatrix} 3\\ 2\end{pmatrix}  \cdot  \begin{pmatrix} 2\\ 0.5\end{pmatrix} = 3\cdot 2 + 2\cdot 0.5 =7\\
&\overrightarrow{v}_{\text{kat}}\cdot \overrightarrow{k}_{\text{lufte}} =\begin{pmatrix} 2\\ 3\end{pmatrix}  \cdot  \begin{pmatrix} 2\\ 0.5\end{pmatrix} = 2\cdot 2 + 3\cdot 0.5 = 5.5  
\end{aligned}
$$
Her er $\overrightarrow{v}_{\text{hund}}\cdot \overrightarrow{k}_{\text{lufte}}$
større end $\overrightarrow{v}_{\text{kat}}\cdot \overrightarrow{k}_{\text{lufte}}$
...
$\overrightarrow{u}_{\text{hund}}$ og $\overrightarrow{u}_{\text{logre}}$, ville vi få
samme skalarprodukt
$\overrightarrow{u}_{\text{hund}}\cdot \overrightarrow{u}_{\text{logre}} =  \overrightarrow{u}_{\text{logre}}\cdot \overrightarrow{u}_{\text{hund}}$,
...
-   Antag at vi har lavet 4-dimensionale input- og kontekstvektorer
    ...
    $$
    \begin{aligned}
    \overrightarrow{v}_{\text{hund}}=\begin{pmatrix} 0.5\\2\\1\\-1\end{pmatrix} ,\quad
    \overrightarrow{k}_{\text{pels}}=\begin{pmatrix} 0\\3\\2\\-2\end{pmatrix},\quad
    \overrightarrow{k}_{\text{fjer}}=\begin{pmatrix} 1\\-2\\1.5\\0.5\end{pmatrix}
    \end{aligned}
    $$
    Udregn skalarprodukterne
    $\overrightarrow{v}_{\text{hund}}\cdot \overrightarrow{k}_{\text{pels}}$ og
    $\overrightarrow{v}_{\text{hund}}\cdot \overrightarrow{k}_{\text{fjer}}$. Passer
...
-   Antag at vi har lavet 3-dimensionale input- og kontekstvektorer som
    ...
    $$
    \begin{aligned}
    \overrightarrow{v}_{\text{kat}}=\begin{pmatrix}0\\2\\1 \end{pmatrix},\quad
    \overrightarrow{v}_{\text{hund}}=\begin{pmatrix}0\\1\\1.7\end{pmatrix}, \quad
    \overrightarrow{v}_{\text{mis}}=\begin{pmatrix}0.4\\2\\0.9\end{pmatrix},\quad
    \overrightarrow{v}_{\text{kælk}}=\begin{pmatrix} 2\\-1\\0 \end{pmatrix}
    \end{aligned}
    $$
    Find længden af de fire vektorer. Find vinklen mellem
    $\overrightarrow{v}_{\text{kat}}$ og de tre øvrige vektorer. Stemmer
...
$\overrightarrow{v}_{w}\cdot \overrightarrow{k}_{c}$ er, desto mere
sandsynligt er det, at ordet $w$ har ordet $c$ som kontekst. Hvis der er $V$ antal ord i sproget, kan vi nummerere de mulige kontekstord som $\text{ord}_1,\text{ord}_2,\ldots,\text{ord}_V$. For hvert af dem får vi et skalarprodukt $\overrightarrow{v}_{w}\cdot \overrightarrow{k}_{\text{ord}_i}$ for $i=1,\ldots,V$. Hvis vi betegner sandsynligheden for, at $\text{ord}_i$ er et kontekstord til $w$, med $P(\text{ord}_i\mid w)$, så vil vi gerne have, at følgende er opfyldt.

1. For hvert $\text{ord}_i$ er $0\leq P(\text{ord}_i \mid w)\leq 1$, da sandsynligheder skal ligge mellem 0 og 1.

2. Den samlede sandsynlighed for at få et af de mulige kontekstord skal være 1, dvs. $P(\text{ord}_1\mid w) + P(\text{ord}_2\mid w) + \dotsm + P(\text{ord}_V\mid w ) = 1$.

3. Jo større skalarproduktet $\overrightarrow{v}_{w}\cdot \overrightarrow{k}_{\text{ord}_i}$ er, desto større er sandsynligheden $P(\text{ord}_i\mid w)$ for at få $\text{ord}_i$ som kontekstord.
...
mellem 0 og 1. Vi bruger derfor en funktion på skalarprodukterne for at få dem lavet om til sandsynligheder, der opfylder 1.-3. Den funktion, vi vil bruge, hedder *Softmax*. Hvis $\overrightarrow{y}$ er en vektor med $V$ koordinater, så er $\text{Softmax}\big(\overrightarrow{y}\big)=\overrightarrow{z}$, hvor $\overrightarrow{z}$ er en ny vektor med $V$ koordinater. Den $i$te koordinat i $\overrightarrow{z}$ er givet ved
...
Lader vi $\overrightarrow{y}$ være vektoren med $i$te koordinat
$$y_i = \overrightarrow{v}_{w}\cdot \overrightarrow{k}_{\text{ord}_i}$$
og bruger Softmax på $\overrightarrow{y}$, så får vi en vektor  $\overrightarrow{z}=\text{Softmax}\big(\overrightarrow{y}\big)$, der kan bruges som sandsynligheder. Mere præcist lader vi
$$P(\text{ord}_i\mid w) = z_i = \frac{e^{y_i}}{e^{y_1} + \dotsm + e^{y_V}} = \frac{e^{\overrightarrow{v}_{w}\cdot \overrightarrow{k}_{\text{ord}_i}}}{e^{\overrightarrow{v}_{w}\cdot \overrightarrow{k}_{\text{ord}_1}} + \dotsm + e^{\overrightarrow{v}_{w}\cdot \overrightarrow{k}_{\text{ord}_V}}}$${#eq-sshmodel}
Egenskaberne ved softmax sikrer, at 1.-3. er opfyldt.
...
Antag at
    ...
    $$
    \begin{aligned}
    \overrightarrow{v}_{\text{hund}}=\begin{pmatrix} 0.5\\2\end{pmatrix},\quad
    \overrightarrow{k}_{\text{fjer}}=\begin{pmatrix} 1\\-1 \end{pmatrix}, \quad
    \overrightarrow{k}_{\text{pels}}=\begin{pmatrix} 0\\1\end{pmatrix},\quad
    \overrightarrow{k}_{\text{hund}}=\begin{pmatrix} 4\\-1\end{pmatrix}
    \end{aligned}
    $$
Vi beregner først skalarprodukterne 
$$
\begin{aligned}
\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{fjer}} &= 0.5\cdot 1 + 2\cdot (-1)  =-1.5\\
\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{pels}}&= 0.5\cdot 0 + 2\cdot 1  = 2\\
\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{hund}} &= 0.5 \cdot 4 + 2 \cdot (-1) = 0
\end{aligned}
$$
Derefter anvender vi Softmax for at finde sandsynlighederne
$$
\begin{aligned}
P(\text{fjer} \mid \text{hund}) &= \frac{e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{fjer}} }}{e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{fjer}} }+e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{pels}} }+e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{hund}} }} = \frac{e^{-1.5}}{e^{-1.5}+e^{2 }+e^{0 }} \approx 0.026\\
P(\text{pels} \mid \text{hund}) &= \frac{e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{pels}} }}{e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{fjer}} }+e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{pels}} }+e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{hund}} }} = \frac{e^{2}}{e^{-1.5}+e^{2 }+e^{0 }} \approx 0.858\\
P(\text{hund} \mid \text{hund}) &= \frac{e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{hund}} }}{e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{fjer}} }+e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{pels}} }+e^{\overrightarrow{v}_{\text{hund}} \cdot \overrightarrow{k}_{\text{hund}} }} = \frac{e^{0}}{e^{-1.5}+e^{2 }+e^{0 }} \approx 0.116
\end{aligned}
$$
...
- Lad $\overrightarrow{y}$ være vektoren
$$\overrightarrow{y}= \begin{pmatrix} 1 \\2 \\-1 \end{pmatrix}$$
Beregn $\overrightarrow{z} = \text{Softmax}(\overrightarrow{y})$.
...
-   Antag, at vores ordforråd består af de tre ord \"sommer\", \"sol\" og \"sne\". Vi har lavet en model for sandsynligheder for kontekstord som i @eq-sshmodel, hvor 
    ...
    $$
    \begin{aligned}
    \overrightarrow{v}_{\text{sommer}}=\begin{pmatrix} 1\\1\end{pmatrix},\quad
    \overrightarrow{k}_{\text{sommer}}=\begin{pmatrix} 1\\-1 \end{pmatrix} ,\quad
    \overrightarrow{k}_{\text{sol}}=\begin{pmatrix} 0\\2\end{pmatrix},\quad
    \overrightarrow{k}_{\text{sne}} =\begin{pmatrix} -1\\-2\end{pmatrix}
    \end{aligned}
    $$
    Hvad er sandsynligheden for, at hvert af de tre ord er 
    kontekst til \"sommer\"? Hvilket af de tre ord optræder oftest som kontekst til sommer?
    

:::

Estimation af vektorrepræsentationer {#estimation-af-vektorer .unnumbered}
----------------------

Vi mangler stadig at bestemme vektorrepræsentationerne
$\overrightarrow{v}_{w}$ og $\overrightarrow{k}_{c}$, så modellen i @eq-sshmodel kommer til at passe til virkelig tekst. Her får
...
[^2]: I praksis er ordpar, der forekommer i nærheden af hinanden ikke helt uafhængige. Hvis fx parret $(\text{hund},\text{madskål})$ forekommer, er det mere sandsynligt, at $(\text{hund}, \text{fodre})$ forekommer i nærheden, end hvis der havde stået $(\text{hund},\text{hundesnor})$.
...
\end{aligned}
$$
Da den naturlige logaritme er en voksende funktion, svarer store værdier af $P(\text{data})$ til store værdier af $\ln (P(\text{data}))$, som igen svarer til små værdier af $-\ln(P(\text{data}))$ **FIGUR?**. Hvis vores model er god, skal 
$$L= - \ln(\text{data}) = -\ln(P(c_1\mid w_1)) - \ln(P(c_2\mid w_2)) - \dotsm - \ln(P(c_M\mid w_m)) $${#eq-Ldef}
...
$$
\begin{aligned}
L = &-\ln\left(\frac{e^{\overrightarrow{v}_{w_1}\cdot \overrightarrow{k}_{c_1}}}{e^{\overrightarrow{v}_{w_1}\cdot \overrightarrow{k}_{\text{ord}_1}} + \dotsm + e^{\overrightarrow{v}_{w_1}\cdot \overrightarrow{k}_{\text{ord}_V}}}\right)
- \ln\left(\frac{e^{\overrightarrow{v}_{w_2}\cdot \overrightarrow{k}_{c_2}}}{e^{\overrightarrow{v}_{w_2}\cdot \overrightarrow{k}_{\text{ord}_1}} + \dotsm + e^{\overrightarrow{v}_{w_2}\cdot \overrightarrow{k}_{\text{ord}_V}}}\right)\\
&- \dotsm - \ln\left(\frac{e^{\overrightarrow{v}_{w_M}\cdot \overrightarrow{k}_{c_M}}}{e^{\overrightarrow{v}_{w_M}\cdot \overrightarrow{k}_{\text{ord}_1}} + \dotsm + e^{\overrightarrow{v}_{w_M}\cdot \overrightarrow{k}_{\text{ord}_V}}}\right) 
\end{aligned}
$${#eq-Lfunktion}
...
fx gøre ved hjælp af *gradientnedstigning*, som du kan læse mere om her. **LINK**
...
Når vektorerne $\overrightarrow{v}_{w}$ er fundet med Word2Vec, kan man bruge dem til at lave algoritmer til at generere tekst. Det ser vi på i næste afsnit. 
...
For at finde minimum for tabsfunktionen, får man brug for at finde partielle afledte af Softmax-funktionen. Det kan man gøre smart ved hjælp af nedenstående opgave, som viser, at man ikke behøver at kende værdien af $\overrightarrow{y}$ i det punkt, hvor man differentierer, men kun funktionsværdien $\overrightarrow{z}$.
...
vektorerne $\overrightarrow{v}_{\text{Hunden}}$, $\overrightarrow{v}_{\text{spiser}}$
og $\overrightarrow{v}_{\text{sit}}$. Disse tre vektorer giver vi funktionen
som input, og som output får vi et nyt ord. Det kunne være \"kødben\". Den
funktion, der bruges i punkt 2., kunne fx være et *neuralt netværk*.
Du kan læse mere om, hvordan det fungerer i \"Tekstgenerering med
neurale netværk\".**LINK**
