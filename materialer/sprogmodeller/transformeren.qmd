...
Chat-GPT står GPT for eksempel for \"Generative Pre-trained Transformer\".
...
gangen. Hvis vi for eksempel har genereret teksten

$$
\text{"Min hund er sulten, så jeg fodrer den med ---"}
$$ 

så skal vi lave en algoritme, der gætter næste ord. Både $N$-gram modeller og
...
forskellige ting, for eksempel \"hø\" eller \"fuglefrø\". I dette tilfælde er vi
...
$\vec{v}_{\text{ord}}$ repræsentere betydningen af ordet. For
...
$\vec{v}_{\text{ord}}$ blev brugt som en fast vektor uanset
hvilke ord, der stod i nærheden af det. Men betydningen af et ord kan
ændre sig alt efter konteksten. Se for eksempel på
$$\text{"Orkesteret skal spille ---"}$$ Kigger vi udelukkende på ordet
\"spille\", kan der stå mange ting bagefter, for eksempel \"ludo\", \"tennis\"
...
vektor $\vec{v}_{\text{spille}}$. Vi modificerer derefter
vores vektorrepræsentation $\vec{v}_{\text{spille}}$ til en
ny vektor $\vec{w}_{\text{spille}}$ afhængigt af hvilke ord,
der kommer før. Hvis ordet \"orkesteret\" kommer inden, så kan man
forestille sig, at det trækker $\vec{v}_{\text{spille}}$ i en
musikretning, mens hvis ordet havde været \"landsholdet\", så ville det
trække $\vec{v}_{\text{spille}}$ i en sportsretning.

Vi vil modificere $\vec{v}_{\text{spille}}$ ved hjælp af
...
Geometrisk lægger man vektorerne $\vec{a}$ og
$\vec{b}$ sammen ved først at tegne $\vec{a}$ og
derefter tegne $\vec{b}$ med udgangspunkt, der hvor
$\vec{a}$ slutter. Vektoren
$\vec{a} + \vec{b}$ er så den vektor, der starter
samme sted som $\vec{a}$ og slutter der hvor
$\vec{b}$ slutter.

Dette kan vi nu benytte til at opdatere vektoren for \"spille\", når vi
ved, at \"orkesteret\" står inden. Hvis vi har vektorerne
$\vec{v}_{\text{spille}}$ og
$\vec{v}_{\text{orkesteret}}$, så kunne vi opdatere
$\vec{v}_{\text{spille}}$ til
...
$\vec{v}_{\text{spille}}$ er blevet trukket over mod
$\vec{v}_{\text{orkesteret}}$ og væk fra
$\vec{v}_{\text{landsholdet}}$.

Der er dog et andet problem: Den nye vektor
$\vec{w}_{\text{spille}}$ er meget lang i forhold til de
...
\end{pmatrix} = \begin{pmatrix}
    ca_1\\ca_2
\end{pmatrix}$$ Hvis $c>0$, er $c\vec{a}$ en vektor, der
peger i samme retning som $\vec{a}$, men er $c$ gange så
lang. Hvis $c<0$, er vektoren $|c|$ gange så lang som
$\vec{a}$, men peger i modsat retning.
...
Den nye vektor $\vec{w}_{\text{spille}}$ er mere
sammenlignelig med den oprindelige $\vec{v}_{\text{spille}}$
...
$\vec{w}_{\text{spille}}$ blive trukket i retning af
$\vec{v}_{\text{landsholdet}}$ i stedet. Men hvad nu, hvis
...
med vektorrepræsentation $\vec{v}_{\text{ord}}$ ville vi
sætte
...
-   Lad $\vec{v}_1 = \begin{pmatrix} 1\\ 3\end{pmatrix}$ og
    ...
    -   Beregn vektorerne $$\begin{aligned}
        ...
        \vec{w}_3 = \frac{2}{3} \vec{v}_1 + \frac{1}{3} \vec{v}_2 \\\end{aligned}$$

    -   Indtegn $\vec{w}_1$, $\vec{w}_2$ og
        $\vec{w}_3$ i et koordinatsystem sammen med
        $\vec{v}_1$ og $\vec{v}_2$.

    -   Passer resultatet med, at vektoren
        $$c_1\vec{v}_1 + c_2\vec{v}_2$$ med
        $c_1,c_2\geq 0$ ligger tættere på $\vec{v}_1$ jo
        større $c_1$ er?

-   Lad $\vec{v}_1 = \begin{pmatrix} 1\\ 2\\ 0\end{pmatrix}$,
    $\vec{v}_2 = \begin{pmatrix} 1\\-1\\2\end{pmatrix}$ og
...
$\vec{v}_i$. Transformeren opdaterer vektoren
$\vec{v}_{i}$ til $\vec{w}_{i}$ på baggrund af
alle de ord, der går forud. Man kan forstille sig, at vi trækker
vektoren $\vec{v}_{i}$ lidt i retning af alle vektorerne
$\vec{v}_{j}$ med $j<i$ svarende til alle de ord, der går
...
hvor $\vec{v}_1,\ldots,\vec{v}_{i-1}$ er
...
bestemmer, hvor meget $\vec{v}_i$ bliver trukket i retning af
$\vec{v}_j$. Hvis $j$te ord har stor indflydelse på
betydningen af $i$te ord, skal $c_{j,i}$ være stor, så
$\vec{v}_i$ bliver trukket forholdsvis langt over mod
$\vec{v}_j$. Hvis omvendt det $j$te ord ikke indeholder nogen
information om betydningen af $i$te ord, skal $c_{j,i}$ være 0. Dermed
bliver $\vec{v}_i$ ikke trukket i retning af
$\vec{v}_j$. I sætningen
...
hele tre vektorer $\vec{v}_{\text{ord}}$,
$\vec{k}_{\text{ord}}$ og $\vec{q}_{\text{ord}}$.
(Fodnote: I litteraturen kaldes de tre vektorer
$\vec{k}_{\text{ord}}$, $\vec{q}_{\text{ord}}$ og
$\vec{v}_{\text{ord}}$ for key, query og value.) Som i
Word2Vec repræsenterer $\vec{v}_{\text{ord}}$ fokusordets
betydning, og $\vec{k}_{\text{ord}}$ repræsenterer ordet når
...
Hvis $\vec{k}_j \cdot \vec{q}_i$ er meget stor
bliver $c_{j,i}$ tæt på 1, mens meget negative
$\vec{k}_j \cdot \vec{q}_i$ svarer til $c_{j,i}$
tæt på 0.
...
vektorer bruges i beregningen af $\vec{w}_i$. Bemærk at vi
...
&&\vec{v}_3= \begin{pmatrix} 1\\ 1\end{pmatrix}\\\end{aligned}$$
Lad os nu beregne den opdaterede vektor $\vec{w}_3$ for
...
&\vec{k}_3\cdot \vec{q}_3= \begin{pmatrix}2\\0.5\end{pmatrix} \cdot  \begin{pmatrix}1\\2\end{pmatrix} = 2\cdot 1 + 0.5 \cdot 2=3\end{aligned}$$
...
c_{3,3} = \frac{e^{\vec{k}_3\cdot \vec{q}_3}}{e^{\vec{k}_1\cdot \vec{q}_3}+e^{\vec{k}_2\cdot \vec{q}_3}+e^{\vec{k}_3\cdot \vec{q}_3}} = \frac{e^{3}}{e^{2}+e^{0}+e^{3} }\approx 0.705\\\end{aligned}$$
...
vektorerne $\vec{v}_{\text{orkesteret}}$,
$\vec{v}_{\text{skal}}$, $\vec{v}_{\text{spille}}$
og $\vec{w}_{\text{spille}}$ indtegnet i et koordinatsystem.
Det ses at $\vec{w}_{\text{spille}}$ er blevet trukket i
retning af $\vec{v}_{\text{orkesteret}}$ i forhold til
$\vec{v}_{\text{spille}}$.
...
&\text{marsvinet}&&
\vec{q}_3 = \begin{pmatrix}0\\1\end{pmatrix}, &&\vec{k}_3=\begin{pmatrix}-1\\2\end{pmatrix},&&\vec{v}_3= \begin{pmatrix} 1\\ 2\end{pmatrix}\\\end{aligned}$$
Vi vil nu se, at den opdaterede vektor $\vec{w}_3$ for
...
-   Tegn vektorerne $\vec{v}_1$, $\vec{v}_2$,
    $\vec{v}_3$ og $\vec{w}_3$ ind i et
    koordinatsystem og overvej, hvordan $\vec{w}_3$ ligger i
    forhold til $\vec{v}_1$, $\vec{v}_2$,
    $\vec{v}_3$ og $\vec{w}_3$.

-   Tegn vektorerne $\vec{k}_1$, $\vec{k}_2$,
    $\vec{k}_3$ og $\vec{q}_3$ ind i et
...
til tre vektorer $\vec{q}^{k}_{i}$,
$\vec{k}^{k}_{i}$ og $\vec{v}^{k}_{i}$ og beregner
en ny vektor $\vec{w}^{k}_{i}$, hvor $k=1,\ldots,K$. Man kan
...
Outputvektorerne $\vec{w}^{k}_i$ fra de forskellige lag
sættes i forlængelse af hinanden til en samlet outputvektor
$\vec{x}_i$ af længde $Km$. Som det næste køres
$\vec{x}_i$ gennem et neuralt netværk, ligesom vi så i noten
\"Tekstgenerering med neurale netværk\", bortset fra at vi ikke bruger
softmax til sidst. Det vil sige, at koordinaterne i
$\vec{x}_i$ bruges som input i det neurale netværk. Outputtet
...
Når vi så har beregnet outputvektorerne $\vec{y}_i$ fra en
...
transformerblok oversættes vektoren $\vec{y}_i$ (ikke det
oprindelige ord) til tre nye vektorer. Så beregner transformerblokken en
ny $\vec{y}_i$, som kan bruges som input til endnu en
...
[\[fig:transformer\]](#fig:transformer){reference-type="ref"
reference="fig:transformer"}. Den tager vektoren $\vec{y}_i$,
...
bruger vi en fast vektor $\vec{a}_{\text{ord}}$ for hvert ord
og omregner $\vec{y}_i$ til scoren
$$s_{\text{ord}} = \vec{a}_{\text{ord}}\cdot \vec{y}_i$$
Disse scores samles i en vektor $\vec{s}$ med $V$
koordinater, hvor $V$ var antallet af ord i ordforrådet. Endelig
omdannes $\vec{s}$ til en vektor af sandsynligheder ved endnu
engang at bruge softmax-funktionen (se \"Tekstgenerering med neurale
netværk\" LINK).
$$\vec{p} = \text{Softmax}(\vec{s})$$ Når man har
disse sandsynligheder, kan man begynde at generere tekst, for eksempel ved hele
...
bruges $V$ vektorer på formen $\vec{a}_{\text{ord}}$.
...
træningsdata. Til at træne GPT-2 blev der for eksempel brugt 8 millioner
