...
$$
\begin{aligned}
w_0 \leftarrow & w_0 + \eta \cdot \sum_{m=1}^{M} \left (t^{(m)}-
(w_0 + w_1 \cdot x_1^{(m)} + \cdots + w_n \cdot x_n^{(m)}) \right)  \\
w_1 \leftarrow & w_1 + \eta \cdot  \sum_{m=1}^{M} \left (t^{(m)}-
(w_0 + w_1 \cdot x_1^{(m)} + \cdots + w_n \cdot x_n^{(m)}) \right) \cdot \left (x_1^{(m)} \right)\\
&\vdots  \\
w_n \leftarrow & w_n + \eta \cdot  \sum_{m=1}^{M} \left (t^{(m)}-
...
$$
\begin{aligned}
w_0 \leftarrow & w_0 + \eta \cdot \sum_{m=1}^{6} \left (t^{(m)}-
(w_0 + w_1 \cdot x_1^{(m)} + w_2 \cdot x_2^{(m)}) \right)  \\
w_1 \leftarrow & w_1 + \eta \cdot  \sum_{m=1}^{6} \left (t^{(m)}-
(w_0 + w_1 \cdot x_1^{(m)} + w_2 \cdot x_2^{(m)}) \right) \cdot \left (x_1^{(m)} \right)\\
w_2 \leftarrow & w_2 + \eta \cdot  \sum_{m=1}^{6} \left (t^{(m)}-
