...
Derfor når vægten $w_{j,k}$ skal opdateres, så gør vi følgende:
...
Hvis vi ser på @fig-netvaerk_generelt_wjk kan vi se, at $w_{j,k}$ kun har indflydelse på $z_j$ og dermed $o_j$.
...
Derfor kan vi ved hjælp af kædereglen finde den partielle afledede af $E$ med hensyn til $w_{j,k}$ på denne måde[^2]

$$
\frac{\partial E}{\partial w_{j,k}} = \frac{\partial E}{\partial z_j} \cdot \frac{\partial z_j}{\partial w_{j,k}}
$$ {#eq-partialE_w_jk}

[^2]: Da tabsfunktionen afhænger af alle $z$-værdierne, skal man egentlig bruge kædereglen af flere variable, så det bliver $\frac{\partial E}{\partial w_{j,k}} = \sum_{i=1}^J \frac{\partial E}{\partial z_i} \cdot \frac{\partial z_i}{\partial w_{j,k}}$, men $\frac{\partial z_i}{\partial w_{j,k}}=0$ når $i \neq j$ og derfor kan vi nøjes med udtrykket i (@eq-partialE_w_jk).
