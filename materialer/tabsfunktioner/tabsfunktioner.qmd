...
tbl-cap-location: top # Virker ikke lige nu - det er vist en bug
---
...
At bestemme om patienten har kræft eller ej -- det vil sige, om targetværdien er $0$ eller $1$,  beror på faglig ekspertise indenfor det genstandsfelt, hvor AI modellen skal anvendes. I eksemplet med kræft vil det for eksempel være baseret på forskellige diagnostiske tests, som en læge kan bruge til at vurdere, om patienten har kræft[^3].
...
Hele idéen omkring træningsdata, outværdi og tabsfunktion er illustreret i @fig-kort_fortalt herunder.
...
Bemærk, at vi her ganger med $x_{m,i}$, fordi vi skal differentiere summen:

$$
w_0 + w_1 \cdot x_{m,1} + \cdots + w_i \cdot x_{m,i} + \cdots + w_n \cdot x_{m,n}
$$

med hensyn til $w_i$ og det eneste led, der afhænger af $w_i$ er $w_i \cdot x_{m,i}$.
...
Vi søger nu de værdier af $w_0$ og $w_1$, som minimerer tabsfunktionen. Bruges gradientnedstigning (du kan selv prøve ved at bruge [app'en](../apps/perceptron_app.qmd)) her fås[^1]:
...
her skal outputværdien $o_m$ ligger mellem $0$ og $1$. Vi bruger derfor igen sigmoid-funktionen[^2]: 
...
Nu ved vi, at $0<o_m<1$, og derfor er også $0<1-o_m<1$. På @fig-ln_graf ses grafen for den naturlige logartime. Her ses det tydeligt, at hvis $x$ ligger mellem $0$ og $1$, så er $\ln(x)$ negativ.
...
De to brøker har $o_m \cdot (1-o_m)$ som fællesnævner og vi kan derfor sætte på fælles brøkstreg:
...
\end{aligned}
$$
Bemærk her, at $o_m \cdot (1-o_m)$ forkorter ud.
...
Vi vil i vores fiktive eksempel med biomarkør og prædiktion af kræft nu prøve at bruge *cross-entropy* tabsfunktionen sammen med sigmoid som aktiveringsfunktion. Vi skal igen datasættet fra @exm-squared_error og indsætte i *cross-entropy* tabsfunktionen i (@eq-crossentropy). Her bruger vi de to udtryk vi fandt i (@eq-to_muligheder) alt efter om $t_m=0$ eller $t_m=1$:
...
Vi søger nu igen de værdier af $w_0$ og $w_1$, som minimerer tabsfunktionen. Bruges gradientnedstigning (du kan selv prøve ved at bruge [app'en](../apps/perceptron_app.qmd)) her fås[^3]:
...
I @fig-eksempel_squared-error-versus-cross-entropy har vi tegnet de to grafer for de to forskellige sigmoid-funktioner ind i samme koordinatsystem. Her kan man se, at sigmoid-funktionen som er baseret på at minimere *cross-entropy* tabsfunktionen (lyseblå) vokser mere stejlt end sigmoid-funktionen som er baseret på at minimere *squared error* tabsfunktionen (mørkeblå). Det er en fordel i forbindelse med prædiktion, fordi prædiktionen baseret på den lyseblå graf er \"mere sikker\" i sin forudsigelse af om patienten har kræft eller ej. Det stemmer også med de sandsynligheder, som vi beregnede i @exm-squared_error og @exm-cross_entropy, som er opsummeret herunder.
...
Som vi lige har set, er det faktisk ikke helt ligegyldigt, om man bruger den ene eller den anden tabsfunktion. Vi skal i dette afsnit se, at *cross-entropy* sammen med aktiveringsfunktionen sigmoid løser et problem, som *squared error* (også sammen med sigmoid) har. 
...
Problemet opstår, fordi man i forbindelse med gradientnedstigning, starter i et tilfældigt punkt $(w_0, w_1, \dots, w_n)$. Hvis dette punkt svarer til vægte, som resulterer i forkerte prædiktioner, så for eksempel $o_m \approx 0$, men $t_m = 1$, så vil faktoren $o_m \cdot (1-o_m)$ være tæt på $0$. Det betyder, at vægtene næsten ikke bliver opdateret, når vi bruger *squared error*, fordi opdateringsleddene alle er tæt på $0$. Noget tilsvarende gør sig gældende, hvis $o_m \approx 1$ og $t_m = 0$. Dette fænomen kalder man for **slow learning**. 
...
**Ege og Lisbeth:** Grunden til at grafen for sigmoid-funktionen i @fig-eksempel_squared-error-versus-cross-entropy baseret på *cross-entropy* er stejlere - er det mon fordi, at vi med *squared error* slet ikke ramt minimum (pga. slow learning)? Og derfor er den ikke så god til at skelne?

**Ege:** På de to grafer for tabsfunktionerne: Kan vi få indtegnet det rigtige minimum OG det minimum, som vi er endt i?
