...
\begin{aligned}
\hat{P}(F_J) &= \frac{\text{Antal brugere der så alle film i } J}{\text{Samlet antal brugere}}\\
...
$$ \hat{r}=b + p_1x_1 + p_2x_2, $$ {#eq-multipel2}
Her er $\hat{r}$ den prædikterede rating.[^111]
Der indgår tre konstanter $b$, $p_1$ og $p_2$ kaldet  *vægte*[^4], som afhænger af den specifikke bruger.

[^111]: Hatten over $r$'et angiver, at det er en prædikteret værdi, mens et $r$ uden hat betegner brugerens faktiske rating.

::: {#exm-eks3}
En bruger har vægtene $b=2$, $p_1=-0.02$
og $p_2=0.05$. Brugerens ratings prædikteres derfor ved
...
$$\hat{r}= b + p_1\cdot 0 + p_2\cdot 0 = b$$ 
...
$$\hat{r}= b+p_1x_1 + p_2x_2$$ til
$$\hat{r}=b+p_1(x_1+1) + p_2x_2 = b+p_1x_1 + p_2x_2 +p_1$$ Vi ser, at når
...
tænke på $p_1$ som hældningen for $x_1$. 
...
brugerens præferencer. I @exm-eks3 var $p_1$ negativ, mens $p_2$ var positiv, svarende til at brugeren godt kan lide action, men ikke kan lide romantik. Vi så da også, at actionfilmen fik højere rating end den romantiske film.
...
regressionsmodel*. Der indgår tre vægte, nemlig $b$, $p_1$ og
$p_2$. Disse vægte afhænger af brugerens præferencer og er derfor specifikke for den
...
skæring $b$. Vi prædiktere brugerens rating af en film som
$$\hat{r} = b+ p_{1}x_{1} + p_{2}x_{2} + \dotsm + p_{M}x_{M}$$ {#eq-multipel}
Dette kaldes en *multipel lineær regressionsmodel med $M$ variable*.

:::: {.callout-note collapse="false" appearance="minimal"}
### Opgave 3: Den multiple regressionsmodel
Betragt den multiple regressionsmodel for brugeren i @exm-eks3, altså
...
::: {.callout-note collapse="false" appearance="minimal"}
### Opgave 5: Multipel regressionsmodel med én variabel

-   Gør rede for, at en multipel regressionsmodel som i (@eq-multipel) med én variabel, er
...
$$
\hat{r}_{j} = b+p_{1}x_{1j} + p_{2}x_{2j}
$$ {#eq-model}
...
I vores tilfælde søger vi altså værdier af $b$, $p_1$ og $p_2$, så $$
\begin{aligned}
\frac{\partial E}{\partial p_1} = 0\\ \\
\frac{\partial E}{\partial p_2} = 0\\ \\
\frac{\partial E}{\partial b} = 0
\end{aligned}
$$ I opgaven nedenfor bliver du bedt om at beregne de partielle afledede
af $E$. Det fører til $$
\begin{aligned}
...
[^7]: Hvis du ikke kender til partielle afledede, eller du har brug for
...
::: {.callout-note collapse="false" appearance="minimal"}
### Opgave 7: Antal vægte som skal estimeres
...
viser I, at de partielle afledede af et enkelt led i $E$ er givet ved 
