...
![Generelt kunstigt neuralt netværk med flere skjulte lag.](../neurale_net/images/generelt_netvaerk.png){width=75% #fig-long_NN}

I denne note vil vi se på et mere simpelt tilfælde, som det er vist i @fig-long_simple_NN. Her er der kun to skjulte lag, og hver af disse lag består kun af én neuron. Sådanne netværk, hvor de skjulte lag netop kun består af én neuron, vil vi her kalde for **simple kunstige neurale netværk**. På den måde befinder disse netværk sig konceptuelt \"in between\" perceptroner og kunstige neuroner på den ene side og generelle kunstige neurale netværk på den anden. De simple kunstige neurale netværk har den fordel, at de giver en fin forståelse for, hvad neurale netværk er -- herunder hvad **feedforward** og **backpropagation** går ud på -- men samtidig uden, at det bliver alt for matematisk svært.
...
Det betyder, at den første (lysegrønne) neuron i @fig-long_simple_NN sender værdien $y$ videre i netværket, hvor $0<y<1$. Ved den næste neuron i @fig-long_simple_NN (repræsenteret ved den mørkegrønne cirkel), beregnes først den vægtede sum og herefter anvendes igen aktiveringsfunktionen: 
...
For at finde de værdier af vægtene som minimerer tabsfunktionen bruges en metode, som kaldes for **backpropagation**. Det forklarer vi lige om lidt, men vi kan allerede nu afsløre, at vi får brug for at kunne differentiere tabsfunktionen. Vi har tidligere set, at alle outputværdierne $o^{(m)}$ er en sammensat funktion. Derfor bliver tabsfunktionen også en sammensat funktion. Det betyder, at vi får brug for at kunne differentiere sammensatte funktioner, og det siger kædereglen noget om. Men først skal vi se, hvordan kædereglen kan opskrives på en smart måde, hvilket vi skal bruge i forbindelse med backpropagation. 
...
h(x)= f(g(x))
$$
Funkionen $h$ afhænger af $x$, men det sker via den indre funktion $g$ og den ydre funktion $f$. Altså $h$ er sammensat af en indre og en ydre funktion:
...
![Illustration af funktionen $h$ som er sammensat af den indre funktion $g$ og den ydre funktion $f$.](images/sammensatfunktion.png){width=75% #fig-sammensatfunktion}

For at beregne $h(x)$ \"sender\" vi først $x$ ind i \"funktionsmaskinen\" for $g$. Her bliver $g(x)$ beregnet. Denne værdi \"sendes\" så ind i \"funktionsmaskinen\" for $f$ og dermed bliver $f(g(x))$ beregnet, som netop er $h(x)$.
...
Du har nok også lært, at man i stedet for at bruge mærker til at angive, at man har differentieret, kan skrive sådan her (kært barn har som bekendt mange navne):

$$
\frac{dh}{dx} = \frac{df}{du} \cdot \frac{du}{dx}
$$

Denne måde at skrive reglen for at differentiere sammensatte funktioner på kaldes for **kædereglen**. Navnet kædereglen kommer af, at de diffenrentialkvotienter, som indgår på højre side bliver \"kædet\" sammen. Det er nemmest at illustrere med farver:

$$
\frac{dh}{dx} = \frac{df}{\color{#8086F2} du} \color{black}\cdot \frac{\color{#8086F2} du \color{black}}{dx}
$$

Det bliver endnu tydeligere, hvis vi ser på en funktion, som er sammensat af ikke bare to, men tre funktioner:
...
For at differentiere $h$ må vi først bruge kædereglen én gange:

$$
h'(t)= f'(g(x(t))) \cdot \left ( g(x(t))\right)'
$$
For at differentiere den sidste faktor må vi bruge kædereglen endnu en gang:

$$
\left ( g(x(t))\right)' = g'(x(t)) \cdot x'(t)
$$

Det vil sige, at vi samlet set ender med:

$$
h'(t)= f'(g(x(t))) \cdot g'(x(t)) \cdot x'(t)
$$
Bruger vi notation med $dh/dt$ kan ovenstående skrives som:

$$
\frac{dh}{dt} = \frac{df}{dg} \cdot \frac{dg}{dx} \cdot \frac{dx}{dt}
$$
Og med farver bliver det tydeligt, hvorfor der er tale om en **kæde**regel:

$$
\frac{dh}{dt} = \frac{df}{\color{#8086F2} dg } \cdot \frac{\color{#8086F2} dg }{\color{#F288B9} dx} \cdot \frac{\color{#F288B9} dx}{dt}
$$
...
idet den indre funktion $w_0 + w_1 \cdot z^{(m)}$ differentieret med hensyn til $w_1$ bare giver $z^{(m)}$. Vi bruger nu den særlige egenskab ved den afledede sigmoid-funktion i (@eq-afledt_sigmoid) og får
...
Alt i alt gør man altså følgende:

1) Sæt alle vægtene til en tilfældig værdi og vælg en værdi for *learning raten* $\eta$.

2) For alle træningsdataeksempler udregnes $y^{(m)}$, $z^{(m)}$ og $o^{(m)}$ ved hjælp af feedforward ligningerne i (@eq-y_m), (@eq-z_m) og (@eq-o_m).

3) Opdatér $w$-vægtene ved hjælp af opdateringsreglerne for $w$-vægtene.

4) Opdatér $v$-vægtene ved hjælp af opdateringsreglerne for $v$-vægtene.

5) Opdatér $r$-vægtene ved hjælp af opdateringsreglerne for $r$-vægtene.

Gentag punkt 2. - 5. hvor feedforward i 2. hver gang er baseret på de netop opdaterede vægte fra det foregående gennemløb. Opdateringen af vægtene fortsætter indtil værdien af tabsfunktionen næsten ikke ændrer sig.
