...
Det næste, der sker, er, at den første (lysegrønne) neuron i @fig-long_simple_NN sender værdien $y$ videre i netværket, hvor $0<y<1$. Ved den næste neuron i @fig-long_simple_NN (repræsenteret ved den mørkegrønne cirkel), beregnes først den vægtede sum og herefter anvendes igen aktiveringsfunktionen: 
...
1) Outputværdien afhænger af inputværdierne $x_1, x_2, \dots, x_n$:
   ...
   Det er den måde vi forstår feedforward-ligninger på: Vi beregner outputværdien $o$ ud fra inputværdierne $x_1, x_2, \dots, x_n$. Vi kan altså tænke på outputværdien som **en funktion af inputværdierne $x_1, x_2, \dots, x_n$**.
...
2) Outputværdien afhænger også af *alle* vægtene $w_0, w_1, v_0, v_1, r_0, r_1, \dots, r_n$
   ...
   $$
   Her tænker vi på $o$ som **en funktion af vægtene $w_0, w_1, v_0, v_1, r_0, r_1, \dots, r_n$**. Det er denne tankegang, vi skal bruge, når vi skal i gang med at træne netværket. Her er inputværdierne (træningsdata) nemlig givet, mens vi ønsker at justere på vægtene, så netværk bliver så godt som muligt til at forudsige vejret.
...
3) Outputværdien $o$ kan udtrykkes ved hjælp af flere sammensatte funktioner.
   ...
   $$
   o = {\color{#F2B33D} f}(z) = {\color{#F2B33D} f}( {\color{#F2B33D} g}(y)) = {\color{#F2B33D} f} ( {\color{#F2B33D} g} ({\color{#F2B33D} h} (x_1, x_2, \dots, x_n))) 
   $$

   Det er årsagen til, at vi senere får brug for kædereglen, når vi skal i gang med at differentiere.
...
Vi kan nu tage hvert træningsdataeksempel og sende det ind i netværket i @fig-long_simple_NN. Det gør vi ved hjælp af feedforward-ligningerne i (@eq-y), (@eq-z) og (@eq-o). Vi får derfor for hvert træningseksempel beregnet en outputværdi $o$. Outputværdien for det $m$'te træningseksempel vil vi kalde for $o^{(m)}$.
...
For at differentiere $k$ må vi først bruge kædereglen én gange:
...
Sidste lighedstegn følger af (@eq-afledt_sigmoid) og at $y^{(m) = \sigma(r_0 + r_1 \cdot x_1^{(m)} + r_2 \cdot x_2^{(m)} + \cdots + r_n \cdot x_n^{(m)})$. Bemærk her, at hvis vi differentierer med hensyn til $r_0$, så bliver $\frac{\partial y^{(m)}}{\partial r_0}=y^{(m)} \cdot (1-y^{(m)}) \cdot 1$.
...
Træder vi et skridt bagud i netværk opdateres $v$-vægtene således:
...
Vi kan nu sammenfatte opdateringsreglener på denne måde:
...
Alle vægtene er nu opdateret, og vi kan gentage punkt 2 til 4, hvor feedforward i 2 hver gang er baseret på de netop opdaterede vægte fra det foregående gennemløb. Opdateringen af vægtene fortsætter indtil værdien af tabsfunktionen næsten ikke ændrer sig.
...
Så står vi her i dag og vil gerne forudsige vejret i morgen. Kig lige ud af vinduet. Regner det eller ej? Mål luftfugtigheden, temperaturen og lufttrykket. Du har nu fire konkret værdier af $x_1$, $x_2$, $x_3$ og $x_4$. Nu bruger du feedforward-ligningerne i (@eq-y), (@eq-z) og (@eq-o) til at beregne outputværdien $o$.


![](images/paraply.jpg){style='float:right;'  width=50%}
...
* Er $o<0.5$? No worries -- i morgen bliver det ikke regnvejr.
