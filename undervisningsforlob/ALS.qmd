...
Se på de to plots af gennemsnitsmålinger, som du lavede i opgave 3. Hvis du ser på gennemsnitsplottet for den imaginære bevægelse, ser det ud som, at kurven danner et \"V\": Først falder den og dernæst stiger den igen. Kurven for baggrundshjerneaktiviteten ser ud til bare at falde en smule (læg mærke til hvor forskellige de to skalaer på $y$-aksen er).

En idé til to features, som kan beskrive disse to opførelser, er følgende:

For hver måleserie, inddel data i tre (næsten) lige store dele og udregn et gennemsnit af målingerne i hver del (nu arbejder vi altså med gennemsnit for den enkelte måleserie og ikke på tværs af de 15 måleserier som før):
...
Men hvor godt virker det mon? Der vil jo nogle gange ske fejl, fordi alle målinger er behæftet med måleusikkerhed. Det er heller ikke sikkert, at der findes en ret linje, som *altid* kan skille de røde punkter fra de blå. Og selv hvis der er, er det ikke sikkert, at vi baseret på træningsdata har fundet den helt rigtige linje.
...
* Udfyld tabellen herunder -- denne tabel kaldes for en [confusion matrix](/noter/vurdering_model/vurdering_model.qmd#confusion-matrix){target="_blank"}.
...
Den andel (i procent) af punkterne, som er blevet klassificeret korrekt, kaldes for  [klassifikationsnøjagtigheden](/noter/vurdering_model/vurdering_model.qmd#klassifikationsnøjagtighed){target="_blank"} -- eller på engelsk *classification accuracy* (CA).
...
### Opgave 11: Træning af kunstig neuron
...
- Upload datasættet under fanen \"Data\".
- Vælg target-variabel, target-værdi og feature-variable under fanen \"Variable\".
- Tryk på \"Træn model\" under fanen \"Træning\".
- Aflæs vægtene $w_0, w_1, w_2$, der fremkommer til højre. De er koefficienterne for den rette linje, der bruges til at adskille bevægelse fra baggrund: $w_0+w_1x+w_2y=0$, hvor $x$ er værdien af feature 1 og $y$ er værdien af feature 2.
