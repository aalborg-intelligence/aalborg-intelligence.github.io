...
$$
hvor afstanden måles i kilometer, og prisen fås i kroner. Situationen kan illustreres, som vist i [@fig-taxa1]{.nolink}.
...
$$
hvor afstanden måles i kilometer, tiden måles i minutter og prisen fås i kroner. Det kan illustreres sådan her:
...
Idéen er, at man sender nogle inputvariable ind i nettet (vist som de mørkeblå cirkler til venstre i [@fig-NN_simple]{.nolink}), hvor der sker nogle beregninger (de grønne cirkler i midten). Herefter returnerer netværket én eller flere outputværdier (vist som de lyserøde cirkler til højre).

Det kan være meget mere kompliceret, end det vi har vist i [@fig-NN_simple]{.nolink} -- med mange flere knudepunkter i mange lag og med mange forbindelser.
...
Vi kan illustrere funktionen fra \"Taxa2\" med samme billede, hvor en konkret taxatur bliver oversat til to tal: afstand og tid. Det er vist i [@fig-NN_taxa2]{.nolink}.
...
Grafen for et andengradspolynomium er en parabel, og hvis tallet foran $x^2$ er positivt, vender grenene opad. Toppunktets $x$-værdi er derfor den $x$-værdi ($x_T$), der giver den mindst mulige $y$-værdi ($y_T$). Se [@fig-toppunkt]{.nolink}.
...
- Får vi en større $y$-værdi, så er vi på vej væk fra minimum, og vi skal derfor skrue lidt ned for $x$. Se [@fig-toppunkt_larger_y]{.nolink}. 
...
- Får vi en mindre $y$-værdi, så er vi på vej hen mod minimum, og vi skal derfor skrue lidt op for $x$. Se [@fig-toppunkt_smaller_y]{.nolink}.
...
I [@fig-bedste_rette_linje1]{.nolink} har vi indtegnet et eksempel på sådanne fem punkter og en ret linje med ligning
...
På [@fig-bedste_rette_linje2]{.nolink} er denne kvadratsum illustreret. Hvis man ellers har de samme enheder på akserne, vil hver af de fem kvadrerede afstande svare til arealet af de kvadrater, som er indtegnet på figuren. Den bedste rette linje er så den linje, som minimerer det samlede areal af kvadraterne. 
...
I [@fig-eks2]{.nolink} er punkterne indtegnet sammen med linjen med ligning $y=2x+1$.
...
Til sidst har man brug for noget testdata, som ikke er brugt til træningen af det neurale net, men hvor man også kender både input og det ønskede output. Det bruges til at afprøve, om det neurale net fungerer efter hensigten, ved at undersøge om det kan forudsige output i testdata.
...
Herunder er et eksempel på et opnået sæt træningsdata skrevet ind i Excel (du kan hente Excel-filen [her](lever_du_baeredygtigt/data/spgskema_traeningsdata.xlsx)):
...
* Under fanen \"Træning\" sættes \"Iterationer\" til 20000, mens der ikke ændres på resten. Tryk på \"Træn model\". Med vores eksempel på træningsdata giver det følgende resultat:
...
viser, hvordan værdien af tabsfunktionen har ændret sig efter hver justering af vægtene. Vi kan her se, at værdien af tabsfunktionen har stabiliseret sig, som ønsket.
...
Fordelen er, at man eventuelt kan løse problemer, der ikke kan løses på anden vis, blandt andet fordi man kan modellere mere komplicerede funktioner end de lineære. Det kræver dog, at man har adgang til store mængder træningsdata af en god kvalitet.

::: {.callout-note collapse="false" appearance="minimal"}

### Ekstraopgave 12: Bæredygtighed og musiksmag

Spørgsmålene i spørgeskemaet, du lavede ovenfor, var nok alle på en form, hvor svarmulighederne tydeligt havde noget med graden af bæredygtighed at gøre. De tillagte point kunne derfor tildeles som $-2$ for det mindst bæredygtige svar op til $+2$ for det mest bæredygtige svar. Dermed vil en simpel udregning af det gennemsnitlige antal point være en god og nok bedre måde at afgøre, om du lever bæredygtigt.

Man undgår dermed den uklare fortolkning af, hvad der foregår ved en anvendelse af et neuralt net.

* Formuler nu igen et spørgeskema, men nu med spørgsmål, som alle omhandler personernes præferencer og kendskab til musik. Igen skal spørgsmålene have fem svarmuligheder, som tillægges point, og for både træningsdata og testdata skal vi kende resultatet $1$, hvis personen lever bæredygtigt og $0$, hvis personen ikke gør.


Det er uklart, om kendskab til en persons musiksmag indeholder nok information til, at man med nogen sikkerhed kan afgøre, om personen lever bæredygtigt. 

* Træn et neuralt net og undersøg, om det virker på testdata.
