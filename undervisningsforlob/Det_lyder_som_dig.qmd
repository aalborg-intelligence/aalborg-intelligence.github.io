...
* Åbn denne fil [Model i Orange](Det_lyder_som_dig_filer/Det_lyder_som_dig_Orange.zip). Den indeholder stukturen til at lave modellen, men du skal indlæse data.
...
* Sæt ID er sat til meta (modellen skal ignorere denne værdi), sæt category (Marlene, Lisbeth, Ege eller Jan) til target, og sæt de 13 MFCC data til feature (det står de nok allerede som). 
...
Når vi før valgte at teste på træningsdata, er det på en måde lidt snyd. Det betyder, at vi først lader modellen træne på træningsdata, og derved tilpasse de mange vægte i modellen ^[Der er 124 vægte i det neurale netværk. Du kan senere lære, hvordan man regner det ud], så den passer bedst muligt til disse data - og så derefter at se, hvor godt modellen passer til præcis samme data.
...
* Vælg i stedet at teste på testdata. Hvad bliver klassifikationsgraden (CA) nu? Det vil som regel blive ringere end ved test på træningsdata.
...
* Jeres lærer vælger en tekst, som I alle skal læse op. Mindst 2 minutter, og gerne længere. 
...
Indtil nu har vi talt meget lidt det neurale netværk. Vi vil på ingen måde gå i detaljer, men dog se en lille smule på det. I det neurale netværk er der i første omgang 2 skjulte lag med hver 5 neuroner. Uden at forstå detaljerne, kan vi gøre modellen mere kompliceret, og dermed måske bedre, ved at øge antal neuroner i hvert lag eller ved at lave flere skjulte lag.
...
* Afprøv forskellige tal for antal neuroner og antal skjulte lag (skriv f.eks. 10,10,10 for 3 lag med 10 neuroner hver) både med test on train data og test on test data. Prøv både at opnå en lav klassifikationsgrad og en høj klassifikationsgrad.
...
* Er der stadig lydklip, som misklassificeres i den model med størst klassifikationsgrad? Lyt i så fald til disse lydklip

:::  

Tillykke - du kender nu nogle af de begreber, som indgår i neurale net, der er en meget vigtig del af AI, og har en ide om, hvordan de anvendes i Orange. Hvis du får lyst til at vide meget mere, er følgende længere forløb et rigtigt godt sted at starte [Kunstig neuron](../undervisningsforlob/kunstige_neuroner_langt_forlob/kunstige_neuroner_forside.qmd)