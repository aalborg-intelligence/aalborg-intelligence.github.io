...
+ Cirklens ligning i planen på formen $(x-a)^2+(y-b)^2=r^2$.
...
-   Indtegn punkterne $\big(x_{1}^{(i)},x_{2}^{(i)}\big)$ for $i=1,\ldots,6$ fra datasættet i tabellen ovenfor i et koordinatsystem, og farv dem alt efter, hvilken klasse de tilhører (hvis $t=1$ kan du for eksempel farve punktet rødt og ellers blåt).
...
- For hvilket af de to punkter er vi mest sikre på klassifikationen?
