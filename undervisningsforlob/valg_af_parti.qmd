...
## Softmax modellen
Vi starter med et meget lille eksempel, hvor 4 personer, Andy, Bella, Charlie og Doresa har svaret på 2 spørgsmål samt angivet, hvilket parti de stemmer på.

| Navn | Spørgsmål 1 $(x_1)$ | Spørgsmål 2 $(x_2)$ | Parti (t) |
|-------|:---------------------------|:-----------:|:-----------:|
| Andy | $\color{red}{\textbf{ Helt uenig } (-2)}$ |  $\color{red}{\textbf{ Helt uenig } (-2)}$ | 1 |
| Bella | $\color{red}{\textbf{Uenig } (-1)}$ | $\color{green}{\textbf{Enig } (1)}$ | 2 |
| Charlie | $\color{red}{\textbf{Uenig } (-1)}$ |  $\color{red}{\textbf{Uenig } (-1)}$ | 2 |
| Doresa | $\color{green}{\textbf{Enig } (1)}$ |  $\color{green}{\textbf{Helt enig } (2)}$ | 3 |
...
Men modellen skulle jo ikke kun give en score, men faktisk give en procent for hvert parti. Så vi skal have fundet en metode til at få procenter i stedet for scores. Om procenterne ved vi, at de skal være positive, at de skal være mellem 0 og 1, og endeligt, at det skal give 1 tilsammen.

En simpel måde at få tallene positive på er at tage $e$ opløftet i scoren. 
...
Denne metode til at omregne fra score to procent kaldes for **Softmax**.
...
Lad os første omgang se på $w_1$ og dermed $x_1$. Bemærk, at $x_1 = -2$ for Andy, at $x_1 = -1$ for Bella og Charlie, og at $x_1 = 1$ for Doresa. Da vi gerne vil have Andy til at få en høj score, og dermed en høj sandsynlighed for parti 1, giver det mening at lade $w_1$ for parti 1 være en negativ værdi, så $w_1 \cdot w_2$ bliver postiv. Før havde vi sat $w_1=-1$, men lad os forstærke effekten med $w_1 = -3$ for parti 1, så scoren for Andy bliver højere. 
...
Det giver så desværre både Bella og Charlie en positiv score, da $w_1 \cdot x_1 = -3 \cdot (-1) = 3$, hvilket ikke er så godt, da vi gerne vil have, at modellen ikke peger på parti 1 for dem. Det venter vi lige med at gøre noget ved, da vi starter med at få modellen til at pege på parti 3 for Doresa.

::: {.callout-note collapse="false" appearance="minimal"}
### Opgave 2

- Overvej hvilken værdi $w_1$ for parti 3 kan gives, så Doresa får en positiv score, mens de øvrige tre får en negativ score? Vælg vægten, så scoren for Doresa for parti 3 til at blive $6$, som scoren for Andy blev for parti 1.
...
Vi skal nu have modellen til at vælge parti 2 for Bella og Charlie, men uden at ødelægge, at modellen vælger parti 1 til Andy og parti 3 til Doresa. 

::: {.callout-note collapse="false" appearance="minimal"}
### Opgave 3

- Overvej hvilken værdi biaset $w_0$ for parti 2 kan gives, så Bella og Charlie får scorer højst for parti 2 (scoren skal over 3, da de allerede har scoren 3 for parti 1), men uden scoren for Andy eller Doresa bliver højst for parti 2 (de har begge 6 som højst score til hhv. parti 1 og parti 3).
...
Med det meget lille, simple eksempel kunne vi altså godt justere vægtene manuelt og få et nogenlunde resultat. Men hvis der bliver 100, 1000 eller millioner af vægte, så er det næppe en god ide. Det kræver noget lidt mere advanceret matematik, som vi ikke vil gennemgå i dette forløb. Du kan læse mere om dette i denne note om [Kunstige neurale netværk til multipel klassifikation]{https://aimat.dk/noter/softmax/softmax.html}

I stedet vil vi bruge en app til formålet. Du finder appen [her](https://apps01.math.aau.dk/ai/multi-neuron/) og datasættet som excelfil [her](/undervisningsforlob/valg_af_parti/datasæt_4_personer.xlsx).
...
Lad os se på et lidt større eksempel, nu med autentiske data fra kandidattest til folketingsvalget i 2026. Vi ser på 16 kandidater og deres svar på 2 spørgsmål.
...
Datasættet er [her](/undervisningsforlob/valg_af_parti/datasæt_752_personer.xlsx).
