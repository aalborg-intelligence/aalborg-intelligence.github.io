...
Læs afsnittene [Opmærksomhed](../../noter/sprogmodeller/transformeren.qmd#opmærksomhed) og [Vektorregning](../../noter/sprogmodeller/transformeren.qmd#vektorregning) eller se videoen her:
...
Læs afsnittet [Et opmærksomhedslag](../../noter/sprogmodeller/transformeren.qmd#et-opmærksomhedslag) eller se videoen her:
...
Vi vil nu se, at den opdaterede vektor $\vec{w}_3$ for \"marsvinet\" bliver trukket meget i retning af vektoren for \"svømmer\", men i mindre grad i retning af \"der\".
...
Læs afsnittene [En transformerblok](../../noter/sprogmodeller/transformeren.qmd#en-transformerblok) og [Transformernetværket](../../noter/sprogmodeller/transformeren.qmd#transformernetværket) eller se videoen her:
...
2. Det første der sker i transformerblokken, er, at vores tokens oversættes til de 3 vektorer $\vec{q}$, $\vec{k}$ og $\vec{v}$.  Husk på, at hver token allerede er blevet oversat til en $768$-dimensional vektor i 1., så i virkeligheden er det disse vektorer, der skal oversættes videre til de tre vektorer.  I GPT-2 havde hver af de 3 vektorer dimension $64$. 
For at lave $\vec{q}$-vektorerne, oversættes hver $768$-dimensional vektor
$$\vec{a}= \begin{bmatrix} a_1\\ \vdots\\ a_{768} \end{bmatrix}$$
til en $64$-dimensional vektor
$$\vec{q}= \begin{bmatrix} q_1\\ \vdots\\ q_{64} \end{bmatrix}$$
ved hjælp af $64\cdot 768$ vægte på formen $c_{ij}^q$, hvor $i=1,\ldots, 64$ og $j=1,\ldots, 768$.
Den $i$'te koordinat i $\vec{q}$ beregnes ved
$$q_i=c_{i1}^qa_1 + c_{i2}^qa_2 + \dotsm + c_{i768}^qa_{768}.$$
Tilsvarende skal der bruges $64\cdot 768$ vægte $c_{ij}^k$ for at lave $\vec{k}$-vektorerne og $64\cdot 768$ vægte $c_{ij}^v$ for at lave $\vec{v}$-vektorerne.
Hvor mange vægte bruges der i alt (bemærk, at det er de samme vægte $c_{ij}^q$, $c_{ij}^k$ og $c_{ij}^v$, der benyttes for alle tokens).

Transformerblokkene sættes sammen til et opmærksomhedslag:

3. I GPT-2 var der $K=12$ parallelle opmærksomhedslag. Gang resultatet fra punkt 2. med $12$ for at få antallet af vægte, der samlet set skal bruges til at lave vektorrepræsentationerne i alle opmærksomhedslagene.
...
Læs afsnittet [Træning af netværket](../../noter/sprogmodeller/transformeren.qmd#træning-af-netværket). 
