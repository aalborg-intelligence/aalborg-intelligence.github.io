...
I det første vindue er \"Hunden\" fokusord, mens \"sover\" er kontekstord. I det andet vindue er \"Hunden\" kontekstord, mens \"sover\" er fokusord. Bemærk, at det er forsimplet i forhold det, hvordan det gøres i Word2vec.

Den overordnede er idéen så, at vi skal bestemme værdier til fokus- og kontekstvektorer for alle fem ord, så værdien af tabfunktionen minimeres.
...
\vec{v}_{spinder} &= \begin{pmatrix} 0.6 \\ 0.8 \end{pmatrix}, \quad
...
\color{#8086F2}
\begin{aligned}
\vec{k}_{hunden} &= \begin{pmatrix} 0.8 \\ -0.6 \end{pmatrix}, \quad
\vec{k}_{katten} = \begin{pmatrix} 0.6 \\ -0.8 \end{pmatrix},  \\
\vec{k}_{spinder} &= \begin{pmatrix} -1 \\ 0 \end{pmatrix}, \quad
...
Som det næste skal sandsynlighederne for at et eller andet ord er kontekstord til et andet udregnes. For eksempel udregnes sandsynligheden for at \"sover\" er kontekst til \"hunden\" ud på denne måde:
...
Endeligt skal værdien af tabsfunktionen beregnes. Her skal vi bruge den naturlige logaritme på de beregnede sandsynligheder. Da \"Hunden sover\" optræder to gange, kan værdien for $\ln(P(\text{sover} \mid \text{hunden}))$ og for $\ln(P(\text{hunden} \mid \text{sover}))$ ganges med 2, \"Hunden gør\" optræder 3 gange, så her gælder noget tilsvarende. Vi får:

$$
\begin{multline}
L= -2 \cdot \ln(P(\text{sover} \mid \text{hunden})) \\ 
-2 \cdot \ln(P(\text{hunden} \mid \text{sover})) -3 \cdot \ln(P(\text{gør} \mid \text{hunden})) - \cdots 
\end{multline}
$$ {#eq-tabsfkt}
