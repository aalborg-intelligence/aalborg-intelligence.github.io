...
description: "En funktion kan afhænge af flere forskellige variable end én, og denne type af funktioner viser sig at spille en helt central rolle i rigtig mange metoder inden for kunstig intelligens."
aliases: 
  - "/materialer/funktioner_af_flere_variable/funktioner_af_flere_variable.html"
from: markdown+emoji
---
...
f(x)=a \cdot x+b.
$$
Grafen for $f$ består af alle de punkter $(x,y)$, hvor $y=f(x)$. Det er illustreret på [@fig-graf_lin]{.nolink}.
...
$$ 
Du har måske også været vant til at se en \"maskine-metafor\", som illustreret på [@fig-maskine]{.nolink}.

[^1]: Definitions- og/eller værdimængden for funktionen behøver ikke at være hele $\mathbb{R}$, men kan godt bare være en delmængde af $\mathbb{R}$.
...
Grafisk er det illustreret på [@fig-grafer]{.nolink}. 
...
\end{aligned}
$$
Det betyder, at punktet $P(1,-2,-7)$ ligger på grafen for $f$. Grafen for $f$ og punktet $P$ ses i [@fig-fun2]{.nolink} herunder.
...
Vi kan igen bruge \"maskine-metaforen\" for funktioner af to variable, som illustreret i [@fig-maskine2]{.nolink}.
...
### VIDEO: Partielle afledede 

I videoen her giver vi en introduktion til funktioner af to variable og partielle afledede.
...
Det er jo alt sammen fint nok, men hvad skal man mon bruge det til? Det er nemmest at forklare, hvis du forestiller dig, at grafen for $f$ er et landskab, hvor du står i punktet $P(x_0,y_0,f(x_0,y_0))$. Så viser det sig, at gradienten $\nabla f(x_0,y_0)$ peger i den retning, hvor funktionsværdien vokser mest[^5]. Det vil altså sige, at hvis du står i $P$ og gerne vil gå allermest opad bakke, så skal du gå i gradientens retning. Det svarer derfor til at følge den snitkurve, som er markeret på den interaktive figur ovenfor. Hvis du drejer rundt på grafen i den interaktive figur, kan du måske få en fornemmelse af, at snitkurven netop angiver den \"sti\", man skal følge, hvis man i punktet $P$ vil gå mest opad bakke. 
...
En virksomhed producerer en bestemt vare. Virksomhedens fortjeneste afhænger af en række faktorer. Fx antal solgte varer, løn til medarbejdere, elprisen, prisen på en bestemt råvare (som bruges i produktionen) og antal sygedage blandt medarbejderne. Virksomheden har fundet ud af, at fortjenesten tilnærmelsesvist kan udregnes på følgende måde[^8]:
