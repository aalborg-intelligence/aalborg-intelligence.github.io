...
Det er illustreret på [@fig-spille]{.nolink}. Det ses, at retningen på $\vec{w}_{\text{spille}}$ er blevet trukket over mod $\vec{v}_{\text{orkesteret}}$ og væk fra $\vec{v}_{\text{landsholdet}}$.
...
Dette er skitseret i [@fig-spille2]{.nolink}.
...
På [@fig-eks1]{.nolink} er vektorerne $\vec{v}_{\text{orkesteret}}$, $\vec{v}_{\text{skal}}$, $\vec{v}_{\text{spille}}$
...
Tilsammen udgør de parallelle opmærksomhedslag og det efterfølgende neurale netværk en *transformerblok*. Strukturen er skitseret på [@fig-transformerblok]{.nolink}. Undervejs sker der forskellige transformationer af vektorerne, som viser sig at gøre de numeriske beregninger mere stabile. Vi vil ikke beskrive disse i detaljer, da de
...
Egentlig var vi jo ude på at lave en algoritme, der kan gætte det næste ord, altså det $(i+1)$te ord. Det sker i den sidste prædiktionsblok i transformernetværket, se [@fig-transformer]{.nolink}. Vektoren $\vec{y}_i$, der kommer ud fra den sidste transformerblok, indeholder information om betydningen af $i$te ord samt alle de ord, der går forud. I prædiktionslaget omregnes $\vec{y}_i$ til sandsynligheder for næste ord.
