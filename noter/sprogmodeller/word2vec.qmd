...
description: "Idéen bag Word2Vec går ud på at repræsentere hvert ord i et sprog med en vektor, hvor ord, hvis betydning minder om hinanden, svarer til vektorer, med nogenlunde samme retning og længde."
aliases: 
  - "/materialer/sprogmodeller/word2vec.html"
---
...
illustreret på [@fig-vektorer]{.nolink}.
...
og noterer dets fire kontekstord (eller færre, hvis vi er i starten eller slutningen af sætningen). Vi
...
[@tbl-data]{.nolink}. Hver række i tabellen består af et fokusord og et af dets kontekstord.[^4]
...
### VIDEO: Introduktion til Word2Vec

I videoen her forklarer vi idéen bag Word2Vec.

{{< video https://www.youtube.com/embed/DVGvvXHI7wQ >}}


I de næste to afsnit ser vi på, hvordan man kan bruge vektorer til at modellere sandsynligheden for, at et fokusord $w$ har $c$ som kontekstord. Det vil vi sidenhen benytte til at vælge vektorerne, således at sandsynlighederne matcher, hvor hyppigt vi observerer $c$ som kontekstord til $w$  i datasættet.
...
vinklen $v$ er, desto
mindre vil $\cos(v)$ være -- se [@fig-cos]{.nolink}.
...
### VIDEO: Fokus- og kontekstvektorer

I videoen her forklarer vi idéen og intuitionen bag fokus- og kontekstvektorer.

{{< video https://www.youtube.com/embed/OdiOc22SLh8 >}}
...
\end{aligned}
$$
Vektorerne er vist i [@fig-eks1]{.nolink}, hvor fokusvektorer er lyserøde, og
...
sandsynligt er det, at ordet $w$ har ordet $c$ som kontekst. Hvis der er $V$ antal ord i sproget, kan vi nummerere de mulige kontekstord som $\text{ord}_1,\text{ord}_2,\ldots,\text{ord}_V$. For hvert af dem får vi et skalarprodukt $\vec{v}_{w}\cdot \vec{k}_{\text{ord}_i}$ for $i=1,\ldots,V$. Hvis vi betegner sandsynligheden for, at $\text{ord}_i$ er et kontekstord til $w$, med $P(\text{kontekst = ord}_i\mid \text{fokus = }w)$, så vil vi gerne have, at følgende er opfyldt:

1. For hvert $\text{ord}_i$ er 
   $$0\leq P(\text{kontekst = ord}_i \mid \text{fokus = } w)\leq 1$$ 
...
3. Jo større skalarproduktet $\vec{v}_{w}\cdot \vec{k}_{\text{ord}_i}$ er, desto større er sandsynligheden $P(\text{kontekst = ord}_i\mid \text{fokus = } w)$ for, at $\text{ord}_i$ er kontekstord til $w$.
...
### VIDEO: Model for sandsynligheder

I videoen her forklarer vi modellen for sandsynligheder, som gennemgået ovenfor.
...
Vi ser nu på $j$te række. Sandsynligheden for, at $w^{(j)}$ har netop $c^{(j)}$ som kontekst, er $P(\text{kontekst = }c^{(j)}\mid \text{fokus = }w^{(j)})$. Vi vil nu finde den samlede sandsynlighed for, at fokusordene $w^{(1)},\ldots,w^{(M)}$ i vores datasæt har henholdsvis $c^{(1)},\ldots,c^{(M)}$ som kontekstord. Den kalder vi $P(\text{data})$. Lad os antage, at alle vores par af fokus- og kontekstord er uafhængige af hinanden[^2]. Så får vi den samlede sandsynlighed ved at gange de enkelte sandsynligheder sammen:
...
Da summer er nemmere at regne på end produkter[^5], vælger vi at tage den naturlige logaritme på begge sider af (@eq-Pdata). Ved at bruge logaritmeregnereglen $\ln(a \cdot b) = \ln(a) + \ln(b)$ får vi, at
$$
\begin{aligned}
...
Da den naturlige logaritme er en voksende funktion, svarer store værdier af $P(\text{data})$ til store værdier af $\ln (P(\text{data}))$, som igen svarer til små værdier af $-\ln(P(\text{data}))$. Dette er illustreret på [@fig-natural_ln]{.nolink}:
...
[^3]: Bemærk, at da $P(\text{kontekst = }c^{(j)}\mid \text{fokus = }w^{(j)})$ er en sandsynlighed, der ligger mellem 0 og 1, er $\ln (P(\text{kontekst = }c^{(j)}\mid \text{fokus = }w^{(j)}))$ negativ. Derfor er $-\ln (P(\text{kontekst = }c^{(j)}\mid \text{fokus = }w^{(j)}))$ positiv og $L$ er dermed også positiv. Når $L$ gerne skal være lav, betyder det altså, at den skal være så tæt på 0 som muligt. 
...
\end{aligned}
$${#eq-Lfunktion}
Bemærk her, at $w^{(j)}$ og $c^{(j)}$ refererer til henholdsvis fokus- og kontekstordet i den $j$'te række i datasættet, mens $\text{ord}_i$ refererer til det $i$'te ord i ordforrådet.
...
### VIDEO: Estimation af vektorrepræsentationer

I videoen her forklarer vi, hvordan vi opstiller tabsfunktionen $L$, som minimeres for at bestemme en vektorrepræsentation for alle fokus- og kontekstord.

{{< video https://www.youtube.com/embed/940yYmus8X8 >}}
...
### VIDEO: Fra vektorer til tekstgenerering

Den overordnede idé med at bruge ords vektorrepræsentationer til at prædiktere det næste ord i en sætning er forklaret i videoen her.

{{< video https://www.youtube.com/embed/eKI2Nd-s9Cw >}}

## Relaterede forløb

* [Word2vec](../../undervisningsforlob/word2vec.qmd){target="_blank"}.
