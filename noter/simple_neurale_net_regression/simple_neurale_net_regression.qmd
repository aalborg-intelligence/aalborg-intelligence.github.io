---
title: "Simple kunstige neurale netværk til regression"
aliases: 
...
* $t$: boligens salgspris målt i millioner kroner
...
Hvis du har læst noten om [kunstige neuroner](../kunstige_neuroner/kunstige_neuroner.qmd){target="_blank"}, så er den eneste forskel her, at vi ikke længere bruger sigmoid-funktionen som aktiveringsfunktion[^1], fordi vi ikke her har brug for at få en outputværdi, som ligger mellem $0$ og $1$.
...
For at bestemme de værdier af vægtene, som minimerer tabsfunktionen, vil vi bruge en metode, som kaldes for gradientnedstigning. Vi har lavet videoer om både [funktioner af to variable](https://youtu.be/tlq2UYWF2Rw){target="_blank"} og [gradientnedstigning](https://youtu.be/WcM8aEoPzf8){target="_blank"}, hvis du vil vide mere. Du kan også læse i noterne om [funktioner af flere variable](/noter/funktioner_af_flere_variable/funktioner_af_flere_variable.qmd){target="_blank"} og [gradientnedstigning](/noter/gradientnedstigning/gradientnedstigning.qmd){target="_blank"}.
...
Man kan træne en kunstig neuron til regression ved at bruge [Kunstig neuron til regression app](https://apps01.math.aau.dk/ai/regressionsneuron/){target="_blank"}.
...
Du kender nok allerede $R^2$-værdien fra lineær regression, som et tal mellem $0$ og $1$, hvor vi gerne vil have tallet så tæt på $1$ som muligt[^R2]. Hvis du vil vide mere om, hvordan $R^2$-værdien rent faktisk regnes ud, kan du læse mere [her](../vurdering_model/vurdering_model.qmd#forklaringsgraden-r2-værdien){target="_blank"}.

[^R2]:  Og så alligevel ikke helt -- det kommer vi tilbage til senere.

Et andet mål, som kan bruges til at sammenligne modeller, er **root mean squared error (RMSE)**. Du kan læse en lidt længere forklaring [her](../vurdering_model/vurdering_model.qmd#root-mean-squared-error-rmse){target="_blank"}, men målet er defineret således:
...
Når en models anvendelighed vurderes ved hjælp af mål som RMSE og $R^2$-værdi, så vil det altid være sådan, at jo mere kompliceret en model, man bruger, desto bedre mål vil man få (det vil sige en lille værdi af RMSE og en værdi af $R^2$, som er tæt på $1$). Hvorfor det er tilfældet, kan du læse meget mere om i noten [Overfitting, modeludvælgelse og krydsvalidering](../krydsvalidering/krydsvalidering.qmd){target="_blank"}). Men kort fortalt går det ud på, at en meget kompliceret model kan fange tendenser i data, som i virkeligheden ikke er reelle -- det er bare støj. Et eksempel ses i [@fig-overfitting]{.nolink}. Her er ti punkter indtegnet, hvor der er anvendt lineær regression (den lyseblå linje) og 9. grads polynomiel regression (den lyserøde kurve).
...
Så ser vi alene på $R^2$-værdien og RMSE, kunne vi blive foranlediget til at tro, at vi skulle vælge polynomiel regression. Man skal altså være utrolig varsom med uden videre at sammenligne den type af mål! Men hvad gør man så? Ét bud er at bruge noget, som kaldes for **krydsvalidering**. Princippet er illustreret i [@fig-krydsvalidering]{.nolink}.
...
Bemærk, at hvis vi sammenligner med RMSE, som ikke er baseret på krydsvalidering fra @exm-lejlighed3, så er RMSE blevet en smule højere her. Det er helt forventeligt, at modellen vil klare sig bedre, når den er testet på de data, som den også er trænet på.

:::

Du kan lære mere om risikoen for overfitting i forløbet om [Overfitting og krydsvalidering med polynomiel regression](/undervisningsforlob/OverfitPoly.qmd){target="_blank"}.
...
Som det ses i [@fig-neuralt_net_regression]{.nolink}, er der til hver pil knyttet en vægt (for eksempel $v_1, u_1, w_1$ og så videre), som skal bruges til at udregne outputværdien $o$. Hvis vi for en stund forestiller os, at vi kender alle vægtene, så udregner vi outputværdien $o$ på følgende måde:

Ved hjælp af inputvariablene og $v$-vægtene (de lysegrønne pile på [@fig-neuralt_net_regression]{.nolink}) beregner vi $z_1$:
...
[^sig]: Omvendt har Sigmoid-funktionen den (for en matematiker) fantastiske egenskab, at den er differentiabel, og man kan vise, at $\sigma'(x) = \sigma(x) (1-\sigma(x))$.
...
Grafen for ReLU-funktionen ses i [@fig-relu]{.nolink}.
...
$$
\begin{aligned}
E(v_0, v_1, v_2, & u_0, u_1, u_2,w_0, w_1, w_2) \\ &= \frac{1}{2} \sum_{m=1}^{M} \left (t^{(m)}-
o^{(m)} \right)^2
\end{aligned}
$$
...
3) Vægtene opdatereres:
   ...
   **$v$-vægtene:**
   $$
   v_i^{\textrm{(ny)}} \leftarrow v_i + \eta \cdot \sum_{m=1}^{M} \delta_v^{(m)} \cdot w_1  \cdot x_i^{(m)}
   $$
   hvor
   $$
   \delta_v^{(m)} = 
   \begin{cases}
   0 & \textrm{hvis } v_0 + v_1 \cdot x_1^{(m)} + v_2 \cdot x_2^{(m)} \leq 0 \\
   t^{(m)}-o^{(m)} &    \textrm{ellers} \\
   \end{cases}
   $$
   **$u$-vægtene:**
   $$
   u_i^{\textrm{(ny)}} \leftarrow u_i + \eta \cdot \sum_{m=1}^{M} \delta_u^{(m)} \cdot w_2  \cdot x_i^{(m)}
   $$
   hvor
   $$
   \delta_u^{(m)} = 
  \begin{cases}
  0 & \textrm{hvis } u_0 + u_1 \cdot x_1^{(m)} + u_2 \cdot x_2^{(m)} \leq 0 \\
  t^{(m)}-o^{(m)}  & \textrm{ellers} \\
  \end{cases}
   $$
   I alle tilfælde sætter vi $x_0^{(m)}=1$.
...
Hvis du har brug for lidt mere information om kædereglen og den notation, som vi har anvendt ovenfor, kan du eventuelt se [denne video](https://www.youtube.com/watch?v=nVaAIVXiUjs){target="_blank"}.
...
Vi har trænet et kunstigt neuralt netværk med ét skjult lag med henholdsvis to og ti neuroner. Det kan man gøre ved at bruge [Neuralt netværk app til regression](https://apps01.math.aau.dk/ai/nn-regression/){target="_blank"}. Vi har her valgt henholdsvis $2$ og $10$ neuroner i det første skjulte lag og $0$ i det andet skjulte lag. Learning rate er sat til $0.0001$, antal iterationer til $1000$ og ReLu er valgt om aktiveringsfunktion i de skjulte lag.
...
En anden måde at sammenligne de to modeller på er illustreret i [@fig-pred_neuron]{.nolink} og [@fig-pred_NN10]{.nolink} herunder. I begge figurer er datapunkterne vist sammen med den prædikterede lejlighedspris (her kaldt for prædiktionsfladen), som funktion af lejlighedens areal ($x_1$) og alder ($x_2$). I [@fig-pred_neuron]{.nolink} herunder er den prædikterede pris baseret på en kunstig neuron:
 
```{r}
n_grid <- 100
areal_grid <- seq(30, 370, length.out = n_grid)
alder_grid <- seq(200, 0, length.out = n_grid)
newdata <- expand.grid(areal = areal_grid, alder = alder_grid, pris_mio = 0)
newdata$pris_mio_lm <- predict(fit_lm, newdata, type = "response")
newdata$pris_mio_nn10 <- predict(fit_nn10, newdata, type = "response")[,1]

z_lm <- matrix(newdata$pris_mio_lm, nrow = n_grid, ncol = n_grid, byrow = TRUE)
z_nn10 <- matrix(newdata$pris_mio_nn10, nrow = n_grid, ncol = n_grid, byrow = TRUE)
...
pp <- plot_ly(showscale = FALSE, hovertemplate = hovertemplate) |>
  add_trace(
    x = dat$areal,
    y = dat$alder,
    z = dat$pris_mio,
  ...
  layout(
    ...
    scene = list(
      ...
      xaxis = list(title = "Areal"),
      yaxis = list(title = "Alder"),
      zaxis = list(title = "Pris i millioner")
    )
  )
```
...
```{r}
#| fig-cap: Den prædikterede lejlighedspris (prædiktionsfladen), som funktion af lejlighedens areal ($x_1$) og alder ($x_2$). Den prædikterede pris er her baseret på en kunstig neuron.
#| label: fig-pred_neuron
#| out-width: 95%
#| fig-height: 7
pp |> 
  add_surface(
...
I [@fig-pred_NN10]{.nolink} er den prædikterede pris baseret på et neuralt netværk med et skjult lag med ti neuroner: 

```{r}
#| fig-cap: Den prædikterede lejlighedspris (prædiktionsfladen), som funktion af lejlighedens areal ($x_1$) og alder ($x_2$). Den prædikterede pris er her baseret på et neuralt netværk med et skjult lag med ti neuroner.
#| label: fig-pred_NN10
#| out-width: 95%
#| fig-height: 7
pp |> 
...
Uanset hvilken model vi bruger, minimerer vi tabsfunktionen i (@eq-tabsfunktion). Lidt forenklet kan den skrives:

$$
E = \frac{1}{2} \sum \left (t-o \right)^2
$$
hvor vi altså summerer over hele træningsdatasættet. Her er $t$ den faktiske lejlighedspris i millioner kroner for en given lejlighed i vores træningsdatasæt, og $o$ er den tilhørende prædikterede pris, som så kan være baseret på en mere eller mindre kompliceret model.
...
| Faktisk pris i mio. <br> $t$ | Prædikteret pris i mio. <br> $o$ | $\frac{t}{o}$ | $\log \left (\frac{t}{o} \right )$ | 
|:---:|:---:|:---:|:---:|
| $1$ | $0.9$ | $0.9$ | $0.04576$ |
| $10$ | $9$ | $0.9$ | $0.04576$ |
: {.bordered}
...
* [Kan du bygge funktioner med ReLU?](../../undervisningsforlob/ReLu.qmd){target="_blank"}
* [AI, klimaudfordringer og sammenstyrtede broer](../../undervisningsforlob/regressionsmodeller.qmd){target="_blank"}
