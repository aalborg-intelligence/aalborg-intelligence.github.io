...
:::
   ...
   Bemærk her, at vi kender $z_1^{(m)}$, $z_2^{(m)}$ og $o^{(m)}$ på grund af feedforward, mens alle $u$-, $v$- og $w$-værdierne er de nuværende værdier af vægtene (*inden* opdatering). Læg også mærke til, at når $w$-vægtene er blevet opdateret, så har vi allerede udregnet $\delta^{(m)}$, og at denne størrelse så \"genbruges\", når $v$- og $u$-vægtene opdateres. Det er denne form for \"genbrug\", som i de store kunstige neurale netværk viser sig, at være fuldstændig afgørende for at beregningerne kan gennemføres på en .... START HER!
