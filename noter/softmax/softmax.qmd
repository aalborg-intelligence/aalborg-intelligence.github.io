...
I [@fig-trainingdata]{.nolink} er træningsdata indtegnet i et tre-dimensionelt koordinatsystem. Punkterne har koordinatsæt $(x_1, x_2, x_3)$ og punktets farve angiver valget (mørkeblå svarer til 1, lyseblå svarer til 2, gul svarer til 3 og lyserød svarer til 4).
...
Disse $n$ inputvariable er vist i [@fig-netvaerk_generelt]{.nolink} som lilla cirkler. De grønne cirkler repræsenterer $J$ output neuroner (i vores eksempel er $J=4$, fordi der skal træffes et valg blandt 4 muligheder). 
...
For at forklare den beregning der så foregår, har vi i [@fig-netvaerk_generelt_wj]{.nolink} kun vist de pile, som går fra inputvariablene frem til den $j$'te outputværdi.
...
Værdien $z_j$ er vist i [@fig-netvaerk_generelt_wj]{.nolink} inde i den grønne cirkel. På tilsvarende vis beregnes $z_1, z_2, \cdots, z_J$. Bemærk her, at vægtene er forskellige. For eksempel er
...
Grafen for denne funktion er vist i [@fig-softmax]{.nolink}. Her ses det tydeligt, at værdien af $o_1$ ligger i intervallet $]0,1[$.
...
I [@fig-natural_ln]{.nolink} har vi tegnet grafen for den naturlige logaritme-funktion samt grafen for minus den naturlige logaritme-funktion:
...
Husk nu på, at $t_j=1$. Hvis vægtene i vores neurale netværk er \"indstillet\" godt, så skal $o_j$ være tæt på 1. I [@fig-natural_ln]{.nolink} kan vi se, at det svarer til, at $-\ln(o_j)$ er tæt på $0$. Det betyder, at værdien af tabsfunktionen er lille. 
...
Hvis vi ser på [@fig-netvaerk_generelt_wjk]{.nolink} og (@eq-z_j) kan vi se, at $w_{j,k}$ kun har indflydelse på $z_j$. Til gengæld har alle $z$-værdierne indflydelse på alle $o$-værdierne på grund af udtrykket i (@eq-o_j).
...
Men da $w_{j,k}$ kun har indflydelse på $z_j$ (jævnfør [@fig-netvaerk_generelt_wjk]{.nolink} og (@eq-z_j)), så vil
...
$$
\begin{aligned}
\frac{\partial z_j}{\partial w_{j,k}} &= \frac{\partial }{\partial w_{j,k}} \left ( w_{j,0} + w_{j,1} \cdot x_1 + \cdots +  w_{j,k} \cdot x_k + \cdots +  w_{j,n} \cdot x_n \right) \\ 
&= x_k
\end{aligned}
$$ {#eq-partial_z_j_w_jk}
...
Hvis $i \neq j$, kan man vise (se hints i nedenstående boks), at

$$
\frac{\partial o_i}{\partial z_j} = - o_i \cdot o_j, \quad \quad i \neq j
$$ {#eq-oizj}

og hvis $i=j$, er

$$
\frac{\partial o_i}{\partial z_j} = o_j(1-o_j), \quad \quad i = j
$$ {#eq-oizi}

<div id="hints-diff-softmax"></div>

::: {.callout-tip collapse="true" appearance="minimal"}

## Differentiation af softmax-funktionen

For at differentiere $o_i$, skal vi bruge, at

$$
o_i = \frac{\mathrm{e}^{z_i}}{\sum_{l=1}^J \mathrm{e}^{z_l}}.
$$

Da $o_i$ er udtrykt ved en brøk, får vi brug for kvotientreglen:

$$
\left ( \frac{f}{g} \right )'(x)= \frac{f'(x)  \cdot g(x) - f(x) \cdot g'(x)}{(g(x))^2}
$$

Brug denne regel til at differentiere $o_i$ med hensyn til $z_j$, når $i \neq j$ og med hensyn til $z_i$. 

Hvis du vil gøre det lidt nemmere, så nøjes med at se på det tilfælde, hvor $J=3$. Da er:

$$
o_i = \frac{\mathrm{e}^{z_i}}{\mathrm{e}^{z_1}+\mathrm{e}^{z_2}+\mathrm{e}^{z_3}}
$$
Bestem så for eksempel

$$
\frac{\partial o_1}{\partial z_1}
$$

og 

$$
\frac{\partial o_1}{\partial z_2}
$$

Sammenlign dit resultat med udtrykkene i (@eq-oizj) og (@eq-oizi) og se, at det stemmer.
...
Et netværk vil altid være forholdsvis god til at prædiktere korrekt på de data, som netværket er trænet på. Det kan du læse mere om i noten om [Overfitting, modeludvælgelse og krydsvalidering](../krydsvalidering/krydsvalidering.qmd). Med det i baghovedet kan vi alligevel se fra [@tbl-prediction]{.nolink}, at den prædikterede sandsynlighed for det valg, som hver elev træffer, er høj. For eksempel vælger elev nr. 1 også valg 1 (at blive i klassen), og den prædikterede sandsynlighed for dette valg er $o_1 = 1.000$. 
...
På [@fig-trainingdata_prediction_wrong]{.nolink} er elev nr. 2 fremhævet ved, at det tilhørende punkt er tegnet større. Hvis man drejer lidt rundt på figuren, kan man se, at lige præcis det punkt ligger lidt lavere på $x_3$-aksen end de øvrige mørkeblå punkter, som kan forklare, hvorfor prædiktionen bliver forkert.
...
:::
:::
Punktplot af træningsdata fra [@tbl-prediction]{.nolink}. Punkternes farve angiver valget (mørkeblå svarer til 1, lyseblå svarer til 2, gul svarer til 3 og lyserød svarer til 4). Det punkt, som er tegnet større, svarer til elev 2, hvor netværket ikke prædikterer korrekt.
:::
