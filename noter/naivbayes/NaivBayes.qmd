---
title: "Naiv Bayes klassifikation"
image: "images/bayes_thm.png"
description: 'Denne note handler om klassifikation ved hjælp af den metode, som kaldes for \"Naiv Bayes\".'
aliases: 
  - "/materialer/naivbayes/NaivBayes.html"
nocite: | 
  @*
---

Denne note handler om klassifikation ved hjælp af den metode, som kaldes for **Naiv Bayes klassifikation**.
...
I [@fig-ln]{.nolink} nedenfor er grafen for logaritmefunktionen tegnet for $x\in ]0, 10].$

![Grafen for $f(x)=\ln (x)$.](images/ln.png){#fig-ln}

Vi ved, at $\ln(1) = 0$ (hvilket også kan ses på grafen i [@fig-ln]{.nolink}), samt at for $x<1$ er $\ln(x)<0$, mens for $x>1$ er $\ln(x)>0$.
