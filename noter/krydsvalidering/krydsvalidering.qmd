...
aliases: 
  - "/materialer/krydsvalidering/krydsvalidering.html"
---
...
en god idé at forsøge med en lineær regression? På [@fig-ret_linje]{.nolink} ser
...
### Datasættet
Datasættet fra [@fig-ret_linje]{.nolink} ses i tabellen herunder, hvis du selv vil prøve at lave lineær regression på data. Data kan også hentes som en [Excel-fil her](data/data.xlsx).
```{r}
#| echo: false  
...
I [@fig-residualplot]{.nolink} ses et såkaldt residualplot. Her kan vi tydeligt se, at der er et mønster i den måde, residualerne fordeler sig omkring $x$-aksen. Det er altså tegn på, at en ret linje ikke er velegnet til at beskrive datapunkterne.
...
$x>1.5$. Det svarer til, at residualerne i [@fig-residualplot]{.nolink} først er negative, så positive og dernæst negative igen. Den rette linje i @fig-ret_linje ser heller ikke ud til at følge punkterne særlig godt. Måske en parabel passer bedre på data? 
...
Det ser ud til, at parablen i [@fig-parabel]{.nolink} følger datapunkterne langt bedre. Vi kunne
...
dataeksempel i [@fig-2grad_3grad]{.nolink}. Andengradspolynomiet er indtegnet med rød til
sammenligning.
...
nogenlunde lige godt på vores data. Men i [@fig-2grad_3grad_zoom]{.nolink} har vi zoomet ud på figuren
ovenfor, og her er der en klar forskel:

```{r extrapolate, fig.width=4, fig.height=4}
#| echo: false
#| fig-cap: Plottet fra [@fig-2grad_3grad]{.nolink}, men hvor der nu er zoomet lidt ud. Her ses det tydeligt, at der i "enderne" bliver stor forskel på anden- og tredjegradspolynomiet.
...
med blå på [@fig-2grad_7grad]{.nolink} nedenfor:
...
med nye datapunkter. I [@fig-2grad_7grad_nypunkter]{.nolink} er polynomierne fra før tegnet sammen med 20
...
#| echo: false
#| fig-cap: Andengradspolynomiet (rød) og syvendegradspolynomiet (blå) fra [@fig-2grad_7grad]{.nolink} sammen med 20 nye datapunkter  (grøn), som kommer fra den samme underliggende fordeling, som de sorte datapunkter fra [@fig-2grad_7grad]{.nolink}. 
...
testdata og resten som træningsdata og får en tabsfunktion $E_i(p)$. Idéen er illustreret i [@fig-krydsvalidering]{.nolink}.
...
* [Overfitting og krydsvalidering med polynomiel regression](../../undervisningsforlob/OverfitPoly.qmd){target="_blank"}.
