...
description: 'Historisk set er forløberen til kunstige neurale netværk de såkaldte perceptroner. I denne note behandles perceptroner, og vi ser på opdateringsalgoritmerne \"perceptron learning algoritmen\" og \"ADALINE\" (adaptive linear neuron).'
aliases: 
  - "/materialer/perceptron/perceptron.html"
...
Historisk set er forløberen til kunstige neurale netværk de såkaldte perceptroner. Vi vil her forklare, hvad en perceptron er, og vi vil se på opdateringsalgoritmerne *perceptron learning algoritmen* og *ADALINE* (adaptive linear neuron). For at gøre det tager vi udgangspunkt i et eksempel om kandidattests.
...
kategorier af svar kunne for eksempel oversættes til matematik på denne måde:
...
Vores beslutning vil vi nu også kvantificere ved hjælp af en variabel $t$, som
...
på rød blok\"). En tilfældig indtegnet linje ses på [@fig-punkter_kandidattest_graf1]{.nolink}.
...
Linjen på [@fig-punkter_kandidattest_graf2]{.nolink} har ligning $$\begin{aligned}
...
Det vil vi se nærmere på nu. Vi skal forestille os, at vi har en masse data, som vi så
...
Lad os så se på hvad \"Perceptron Learning Algoritmen\" går
...
-   Udregn fejlen $\textrm{error}$ (som er forskellen mellem den ønskede
...
$$
\begin{aligned}
    w_0^{(\textrm{ny})} \leftarrow w_0 + & \eta \cdot \textrm{error} \\
    w_1^{(\textrm{ny})} \leftarrow w_1 + & \eta \cdot  \textrm{error} \cdot x_1 \\
    w_2^{(\textrm{ny})} \leftarrow w_2 + & \eta \cdot  \textrm{error} \cdot x_2 \\
    & \vdots  \\
    w_n^{(\textrm{ny})} \leftarrow w_n + & \eta \cdot  \textrm{error} \cdot x_n \\
\end{aligned}
$$
...
$$
\begin{aligned}
\textrm{error} = t-o=1-1=0
\end{aligned}
$$
og derfor ender vi også med ikke at opdatere nogle af vægtene, fordi vi generelt har (hvor $x_0=1$):
$$
\begin{aligned}
w_i^{(\textrm{ny})} \leftarrow w_i + \eta \cdot \textrm{error} \cdot x_i,
\end{aligned}
$$
men når $\textrm{error}=0$ ender vi bare med 
$$
\begin{aligned}
w_i^{(\textrm{ny})} \leftarrow w_i.
\end{aligned}
$$
...
$$
\begin{aligned}
\textrm{error} = t-o=-1-1=-2.
\end{aligned}
$$
Vores vægte skal derfor opdateres til
$$
\begin{aligned}
w_0^{(\textrm{ny})} &\leftarrow 0.5 +0.5 \cdot (-2) = -0.5 \\
w_1^{(\textrm{ny})} &\leftarrow 0.5 +0.5 \cdot (-2) \cdot (-1) = 1.5 \\
w_2^{(\textrm{ny})} &\leftarrow 0.5 +0.5 \cdot (-2) \cdot 1= -0.5 \\
\end{aligned}
$$
...
$$
\begin{aligned}
\text{Mulighed 1:} \quad \quad &t=1 \quad \text{og} \quad o=-1 \quad \Rightarrow \quad \textrm{error} = t-o=2 \\
\text{Mulighed 2:} \quad \quad &t=-1 \quad \text{og} \quad o=1 \quad \Rightarrow \quad \textrm{error} = t-o=-2 \\
\end{aligned}
$$
...
på, hvad der sker efter opdateringen af vægtene. Da $\textrm{error}=2$ i mulighed
$1$ har vi opdateringsreglen (hvor $x_0=1$): 
$$
\begin{aligned}
w_i^{(\textrm{ny})} \leftarrow w_i + \eta \cdot \textrm{error} \cdot x_i = w_i + \eta \cdot 2 \cdot x_i.
\end{aligned}
$$
...
mulighed $2$ er fejlen $\textrm{error}=-2$ og opdateringsreglen bliver derfor
$$
\begin{aligned}
w_i^{(\textrm{ny})} \leftarrow w_i + \eta \cdot \textrm{error} \cdot x_i = w_i -\eta \cdot 2 \cdot x_i.
\end{aligned}
...
1.  Hvis der er små fejl i data -- eksempelvis en targetværdi der er
    forkert. Et eksempel herpå ses i figur [@fig-punkter_graf4_5-1]{.nolink}. 
...
2.  Verden er sjældent lineær separabel! Antag, at der ingen fejl er i
    datasættet i figur [@fig-punkter_graf4_5-1]{.nolink}. Alligevel vil det stadig give
    mening at indtegne en ret linje som i figur
    [@fig-punkter_graf4_5-2]{.nolink}, der kan bruges til klassificering.
...
Der er derfor brug for noget lidt andet end den klassiske perceptron. Eller rettere sagt: perceptronen er god nok, det er faktisk bare læringsalgoritmen, som skal justeres lidt. Det viser
...
$$
\begin{aligned}
\textrm{error} = t - (w_0 + w_1 \cdot x_1 + \cdots + w_n \cdot x_n)
\end{aligned}
$$
...
\end{aligned}
$$
i begge tilfælde. Hvis vi i stedet bruger $\textrm{error} = t - (w_0 + w_1 \cdot x_1 + \cdots + w_n \cdot x_n)$ får vi:
$$
\begin{aligned}
&\text{Tilfælde 1:} \quad \textrm{error} = 1-(-0.1)=1.1 \\
&\text{Tilfælde 2:} \quad \textrm{error} = 1-(-5000)=5001
\end{aligned}
$$
...
$\textrm{error} = t - (w_0 + w_1 \cdot x_1 + \cdots + w_n \cdot x_n)$. Dette er
...
$(x_1^{(m)}, x_2^{(m)}, \dots, x_n^{(m)})$ og targetværdi $t^{(m)}$.
...
forskellen mellem den ønskede targetværdi $t^{(m)}$ for det $m$'te
træningseksempel og den vægtede sum
$$
\begin{aligned}
w_0 + w_1 \cdot x_1^{(m)} + \cdots + w_n \cdot x_n^{(m)}, 
\end{aligned}
$$
...
herpå ses i [@fig-tabsfunktion]{.nolink}.
...
Forestil dig at grafen for tabsfunktionen i [@fig-tabsfunktion]{.nolink} er et landskab med en dal. Dit mål er at finde ned i dalen. Du er blevet placeret et tilfældigt sted i landskabet
...
$$
\begin{aligned}
    \textrm{error}_0 &= \sum_{m=1}^{M} \left (t^{(m)}-(w_0 + w_1 \cdot x_1^{(m)} + \cdots + w_n \cdot x_n^{(m)}) \right) \\
    \textrm{error}_1 &= \sum_{m=1}^{M} \left (t^{(m)}-(w_0 + w_1 \cdot x_1^{(m)} + \cdots + w_n \cdot x_n^{(m)}) \right) \cdot x_1^{(m)} \\
    &\vdots \\
    \textrm{error}_n &= \sum_{m=1}^{M} \left (t^{(m)}-(w_0 + w_1 \cdot x_1^{(m)} + \cdots + w_n \cdot x_n^{(m)}) \right) \cdot x_n^{(m)} 
\end{aligned}
$$
...
$$
\begin{aligned}
    w_0^{(\textrm{ny})}  \leftarrow & w_0 + \eta \cdot \textrm{error}_0 \\
    w_1^{(\textrm{ny})}  \leftarrow & w_1 + \eta \cdot \textrm{error}_1 \\
    & \vdots \\
    w_n^{(\textrm{ny})}  \leftarrow & w_n + \eta \cdot \textrm{error}_n 
\end{aligned}
$$
...
\begin{aligned}
 ...
 \end{aligned}
 $$
Linjen er indtegnet sammen med træningsdata i [@fig-punkter_kandidattest_gradientdescent]{.nolink}.
