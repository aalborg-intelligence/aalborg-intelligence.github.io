...
$$
\begin{aligned}
    w_0^{(\textrm{ny})} \leftarrow w_0 + & \eta \cdot error \\
    w_1^{(\textrm{ny})} \leftarrow w_1 + & \eta \cdot  error \cdot x_1 \\
    w_2^{(\textrm{ny})} \leftarrow w_2 + & \eta \cdot  error \cdot x_2 \\
    & \vdots  \\
    w_n^{(\textrm{ny})} \leftarrow w_n + & \eta \cdot  error \cdot x_n \\
\end{aligned}
$$
...
$$
\begin{aligned}
w_i^{(\textrm{ny})} \leftarrow w_i + \eta \cdot error \cdot x_i,
\end{aligned}
$$
...
$$
\begin{aligned}
w_i^{(\textrm{ny})} \leftarrow w_i + \eta \cdot error \cdot x_i = w_i + \eta \cdot 2 \cdot x_i.
\end{aligned}
$$
...
\begin{aligned}
    ...
    error_1 &= \sum_{m=1}^{M} \left (t^{(m)}-(w_0 + w_1 \cdot x_1^{(m)} + \cdots + w_n \cdot x_n^{(m)}) \right) \cdot x_1^{(m)} \\
    &\vdots \\
    error_n &= \sum_{m=1}^{M} \left (t^{(m)}-(w_0 + w_1 \cdot x_1^{(m)} + \cdots + w_n \cdot x_n^{(m)}) \right) \cdot x_n^{(m)} 
\end{aligned}
$$
