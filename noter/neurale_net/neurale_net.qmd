...
description: "I denne note giver vi en grundig gennemgang af matematikken bag kunstige neurale netværk, hvor der i første omgang tages udgangspunkt i et netværk med to skjulte lag hver med to neuroner."
aliases: 
  - "/materialer/neurale_net/neurale_net.html"
...
forenklet er vist på [@fig-pixel]{.nolink}.
...
billeder. Hvis mit billede er et $8 \times 8$ pixels som på [@fig-pixel]{.nolink}, så kan jeg altså
...
[@fig-simple_NN_weights]{.nolink} ser du et sådant simpelt kunstigt
...
$$ {#eq-sigmoid} 
Grafen for sigmoid-funktionen ses i [@fig-sigmoid_graf]{.nolink}. Her kan man se, hvordan ethvert reelt tal bliver afbildet over i et tal mellem $0$ og $1$.
...
Beviset er ikke så svært og overlades til læseren (se eventuelt også forløbet om [aktiveringsfunktioner](../../undervisningsforlob/aktiveringsfunktioner.qmd#sigmoid)){target="_blank"}.
...
for den ser ud, som vist i [@fig-EksE]{.nolink}. Her kan du se, at tabsfunktionen har to lokale minima
...
[@fig-graf_ln]{.nolink}). Alt i alt får vi i dette tilfælde, at $E$ er tæt
...
henover billedet, som vist på [@fig-sliding_window]{.nolink}.
...
netværket. Dette er illustreret på [@fig-local_receptive_fields]{.nolink}.
...
som vi har gjort tidligere). Hvert af vinduerne til venstre i [@fig-local_receptive_fields]{.nolink} kalder man for et *local receptive field* for den tilhørende skjulte neuron til højre i figuren. For at beregne den værdi som den første skjulte neuron sender videre i
...
Et filter, som kan bruges til at finde henholdsvis lodrette og
vandrette linjer i et billede er vist i [@fig-feature1_maps]{.nolink} og [@fig-feature2_maps]{.nolink}.
```{r}
#| label: fig-feature1_maps
#| fig-cap: Et filter $f_1$, som kan finde vandrette linjer i billeder.
...
#| fig-cap: Et filter $f_2$, som kan finde lodrette linjer i billeder.
feature2<-matrix(c(1,1,1,0,0,0,-1,-1,-1),nrow=3)
f2<-hux(feature2)
set_all_borders(f2)
```

Det ses hurtigt, at hvis vi anvender filtret $f_1$ til at vægte
...
filter ikke har fundet nogle vandrette linjer i billedet.
...
det første bruges et filter til at finde egenskaber på et billede
...
lag. Dette er illustreret på [@fig-CNN]{.nolink}.
...
For det andet vil man ikke anvende filtre som på forhånd er
...
benyttes i det pågældende filter. Selve træningen af netværket
