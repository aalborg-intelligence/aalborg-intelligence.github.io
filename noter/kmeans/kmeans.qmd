...
gruppe ligger tæt på hinanden. Det er også sådan, at vi har farvet punkterne til højre i [@fig-kmeansbil]{.nolink}. 
...
Idéen er så nu, at vi vil prøve at bestemme grupperne $S_1, S_2, \dots, S_k$ sådan, at denne sum bliver så lille som muligt. Det vil nemlig svare til, at de punkter, der ligger tæt på hinanden, kommer i samme gruppe, og punkter, som ligger langt væk fra hinanden, kommer i forskellige grupper.
...
Dette er illustreret i [@fig-midterpunkter]{.nolink}.
...
På [@fig-midterpunkter]{.nolink} bliver det tydeligt, hvorfor det er fornuftigt at vælge midterpunkterne, som det sker i trin 2 i algoritmen -- midterpunkterne ligger simpelthen i \"midten\" af hver gruppe.
::: 

## Eksempel på algoritmen

Lad os nu prøve at bruge algoritmen på punkterne fra @exm-midterpunkter. I [@fig-punkter_to_grupper_1]{.nolink} ses punkterne indtegnet, men uden angivelse af hvilken gruppe hvert enkelt punkt tilhører.
...
I trin 1 skal vi tilføje hver observation i en tilfældig gruppe. Et sådant valg ses i [@fig-punkter_to_grupper_2]{.nolink}.
...
$$
Disse to midtpunkter er indtegnet i [@fig-punkter_to_grupper_3]{.nolink} og markeret med et plus.
```{r}
#| echo: false
#| message: false
#| warning: false
...
$$
Resultatet af at beregne alle afstande på denne måde ses i [@tbl-punkter_dist1]{.nolink}.
```{r}
#| echo: false
#| message: false
#| warning: false
...
Vi skal nu afgøre hvilken gruppe, de enkelte punkter skal tilhøre, ved at se på hvilket midtpunkt som hvert enkelt punkt ligger tættest på. For eksempel kan vi i [@tbl-punkter_dist1]{.nolink} se, at det første punkt $(3,9)$ ligger tættest på $\vec{\mu_1}$, og det punkt skal derfor (fortsat) hører til gruppe 1.

I [@tbl-punkter_dist2]{.nolink} ses den oprindelige gruppe samt den nye gruppe for hvert punkt.
...
I [@fig-punkter_to_grupper_4]{.nolink} ses punkterne indtegnet med en angivelse af den nye inddeling (men stadig med de først beregnede midterpunkter).
...
De to nye midtpunkter ses indtegnet i [@fig-punkter_to_grupper_5]{.nolink} -- igen markeret med et plus.
```{r}
#| echo: false
#| message: false
#| warning: false
...
fig3
```
Vi kan nu igen udregne afstande fra alle punkter til de to nye midtpunkter og finde ud af om nogle af punkterne eventuelt skal skifte gruppe. Resultatet ses i [@tbl-punkter_dist3]{.nolink}:
```{r}
#| echo: false
#| message: false
#| warning: false
...
pander(dist_grp)
```
Vi kan nu se, at ingen af punkterne har skiftet gruppe, og algoritmen stopper derfor. Den endelige inddeling i grupper bliver derfor som vist i [@fig-punkter_to_grupper_5]{.nolink}, hvilket nok også er den inddelingen, som vi ville have valgt bare ved at kigge på punkterne med det blotte øje.
...
* [Inddeling af musik i grupper](../../undervisningsforlob/musik.qmd){target="_blank"}.
