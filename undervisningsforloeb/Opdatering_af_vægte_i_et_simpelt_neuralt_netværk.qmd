...
Forudsætningerne kan f.eks. dækkes vha. [note om simple neurale netværk](../materialer/perceptron/perceptron.qmd){target="_blank"}.
...
At forstå gennem detaljerede beregninger, hvordan vægtene i et simpelt neuralt netværk opdateres med brug af sigmoid aktiveringsfunktionen og gradientnedstigning.
...
sigmoid-funktionen $\sigma(x)=\frac{1}{1+e^{-x}}$ som aktiveringsfunktion 
og kvadrat-sum som tabsfunktion $E(w_0, w_1, w_2) = \frac{1}{2} \sum_{m=1}^{3} \left (t_m-\sigma(w_0 + w_1 \cdot x_{m,1} + w_2 \cdot x_{m,2}) \right)^2$ 
...
w_1 \leftarrow & w_1 - 0.1 \cdot 0.05202358001 \approx 0.09479764200  \\
w_2 \leftarrow & w_2 - 0.1 \cdot (-0.0535258408) \approx 0.1053525841 
\end{aligned}
$$
:::
...
Bemærk, at tabsfunktionen er blevet lidt mindre. Formålet er jo netop at minimere den gennem gradientnedstigning, så som regel bør værdien bliver mindre, hver gang vægtene opdateres.
...
Med [Perceptron app](../apps/perceptron_app.qmd){target="_blank"} kan du lave de samme beregninger bare automatisk.

::: {.callout-note collapse="false" appearance="minimal"}
### Opgave 5: Afprøv perceptron app.

* Lav et Excelark med feature- og targetværdierne.
* Indlæs data fra Excelarket i perceptron appen.
* Vælg de korrekte værdier i felterne på appen. Herunder skal feature-skalering være slået fra.
* Se om appen giver samme resultater for tabsfunktionen og vægtene, som du selv fik efter 1 iteration og efter 2 iterationer.

:::

::: {.callout-note collapse="false" appearance="minimal"}
### (Extra) Opgave 6: Cross-entropy tabsfunktion 
* Gentag opgave 1-5 men med cross-entropy som tabsfunktion i stedet for kvadrat-sum.

:::

NB: Ege skal have tilføjet en mulighed for at slå featureskalering fra i appen, før det fungerer efter hensigten.
