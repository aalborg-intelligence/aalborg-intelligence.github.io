...
Vi starter med at indse, at funktionsværdierne for $f$ ligger i $(-1,1)$. Ser vi på definitionen i (@eq-def_softsign), kan vi se, at hvis $x \geq 0$, så vil $f(x) \geq 0$, mens hvis $x<0$ så vil $f(x)<0$ (tælleren i $\frac{x}{1-x}$ er negativ, mens nævneren er positiv). Når vi skal vise, at $f(x)<1$, så kan vi altså nøjes med at se på $x \geq 0$. Men da
...
Vi vil nu vise, at den afledede af softsign-funktionen kan udtrykkes ved hjælp af softsign-funktionen selv:
$$
f'(x)=(1-|f(x)|)^2.
$$
...
Man siger, at linjerne med ligning $y=-1$ og $y=1$ er vandrette asymptoter.
