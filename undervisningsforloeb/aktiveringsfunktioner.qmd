...
Vi vil herefter indse, at funktionsværdierne for $f$ ligger i $(-1,1)$. Ser vi på definitionen i (@eq-def_softsign), kan vi se, at hvis $x \geq 0$, så vil $f(x) \geq 0$, mens hvis $x<0$, så vil $f(x)<0$ (tælleren i $\frac{x}{1-x}$ er negativ, mens nævneren er positiv). Når vi skal vise, at $f(x)<1$, så kan vi altså nøjes med at se på $x \geq 0$. 

Antag derfor først, at $x \geq 0$. Da
...
For at indse at $f(x)>-1$ kan vi nøjes med at se på $x<0$. Nu er
